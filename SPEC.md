...
- Grabbed/thrown victim anchor still uses a suite-focused proxy outside the proven entry subsets. For the decomp-backed `ThrowLw/ThrownLw` handoff slice, the sim resolves the thrown anchor from the thrower’s `FtPart_TransN2` reparent constraint (`ftCo_800DB368`) and applies a narrow entry split where the forward residual collapses onto the new attached joint while the vertical residual is carried from the pre-entry world (`src/grab_flow.c`, `src/grab_attachment.c`). For non-low `Thrown*` entry, the stale pre-entry world-offset bridge is intentionally not used; placement is attachment-owned from static `x1A70` plus the current owner anchor.
