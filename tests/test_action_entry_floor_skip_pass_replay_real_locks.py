...
@pytest.mark.parametrize(
    ...
    [
        (f"{_AGG_VALID}/yoshis_story_recent/PhysicalElectricCapybara.slpz", 157, 0),
        (f"{_AGG_VALID}/yoshis_story_recent/PhysicalElectricCapybara.slpz", 158, 0),
        (f"{_AGG_VALID}/yoshis_story_recent/PhysicalElectricCapybara.slpz", 159, 0),
        (f"{_AGG_VALID}/battlefield_recent/DelayedSuperbGuanaco.slpz", 400, 1),
        (f"{_AGG_VALID}/battlefield_recent/DelayedSuperbGuanaco.slpz", 401, 1),
        (f"{_AGG_VALID}/pokemon_stadium_recent/CornyDelayedOkapi.slpz", 91, 1),
    ],
)
...
def test_pass_floor_skip_fresh_down_repress_can_still_fastfall_synthetic_lock() -> None:
    ...
    root = Path(__file__).resolve().parents[1]
    _skip_if_required_artifacts_missing(root)
    dataset_rel = f"{_AGG_VALID}/yoshis_story_recent/PhysicalElectricCapybara.slpz"
    dataset_path = root / dataset_rel
    if not dataset_path.exists():
        pytest.skip(f"missing local replay: {dataset_rel}")
...
def test_fd_cardinal_attackair_fastfall_latch_non_regression_lock() -> None:
    ...
    root = Path(__file__).resolve().parents[1]
    _skip_if_required_artifacts_missing(root)
    dataset_rel = f"{_AGG_VALID}/cardinal_1.0_recent/AttachedGoodNaturedGuanaco.slpz"
    dataset_path = root / dataset_rel
    if not dataset_path.exists():
        pytest.skip(f"missing local replay: {dataset_rel}")
...
def test_pass_to_attackair_keeps_floor_skip_for_same_callback_replay_real_lock() -> None:
    ...
    root = Path(__file__).resolve().parents[1]
    _skip_if_required_artifacts_missing(root)
    dataset_rel = f"{_AGG_VALID}/fountain_of_dreams_recent/ElatedWearyTermite.slpz"
    dataset_path = root / dataset_rel
    if not dataset_path.exists():
        pytest.skip(f"missing local replay: {dataset_rel}")
