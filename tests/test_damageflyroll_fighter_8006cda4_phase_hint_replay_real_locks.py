...
@pytest.mark.parametrize(
    ...
    [
        ...
        _DamageFlyRoll8006CDA4Case(
            ...
            expect_seed_phase=0,
            expect_action_id=88,
            note="unseparated DamageFlyTop carry stays off this narrow seeded lane (TBK)",
        ),
    ],
)
def test_damageflyroll_fighter_8006cda4_phase_hint_replay_real_locks(
    case: _DamageFlyRoll8006CDA4Case,
) -> None:
    # Replay-real seed and one-step locks for the narrow Fighter_8006CDA4 hidden pre-gate consume
    # bridge.
    ...
    assert (
        int(seed["damageflyroll_fighter_8006cda4_phase_hint"][victim]) == int(case.expect_seed_phase)
    ), case.note

    if int(case.expect_seed_phase) == 1:
    ...
    if int(case.expect_seed_phase) == 2:
