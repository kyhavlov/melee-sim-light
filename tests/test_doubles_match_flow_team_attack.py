...
def _set_buttons(inp: np.ndarray, player: int, buttons: int) -> None:
    inp.view(INPUT_DTYPE).reshape((inp.shape[0],))["p"][0, player]["buttons"] = np.uint16(buttons)
...
def test_pending_rebirth_derivation_frame0_pending_slot_uses_static_char_fallback() -> None:
    post_action = np.array([[ACT_DEAD_DOWN, ACT_WAIT, ACT_WAIT, ACT_WAIT]], dtype=np.uint16)
    post_char = np.array([[0, CHAR_FOX, CHAR_FOX, CHAR_FOX]], dtype=np.uint8)
    post_stocks = np.array([[0, 2, 1, 1]], dtype=np.uint8)
    timer = np.array([[1, 0, 0, 0]], dtype=np.uint8)
    ...
    assert got[0, 0] == CHAR_FOX


def test_pending_rebirth_x2218_derivation_carries_only_reset_surviving_bits() -> None:
    post_action = np.array(
        [
            [ACT_DEAD_LEFT, ACT_WAIT, ACT_WAIT, ACT_WAIT],
            [ACT_DEAD_DOWN, ACT_WAIT, ACT_WAIT, ACT_WAIT],
    ...
    post_flags = np.zeros((3, 4, 5), dtype=np.uint8)
    post_flags[0, 0, 0] = np.uint8(0xF4)
    timer = np.array(
        [
            [2, 0, 0, 0],
            [1, 0, 0, 0],
    ...
    got = _derive_match_flow_pending_rebirth_state_flags_2218(
        post_action_id_u16=post_action,
        post_char_id_u8=post_char,
        post_stocks_u8=post_stocks,
        post_state_flags_u8=post_flags,
        match_flow_timer_u8=timer,
        static_char_id_u8=np.array([CHAR_FOX, CHAR_FOX, CHAR_FOX, CHAR_FOX], dtype=np.uint8),
        team_id_u8=np.array([0, 0, 1, 1], dtype=np.uint8),
        is_teams=True,
    ...
    assert got[:, 0].tolist() == [0, 0x44, 0x44]
...
def test_team_stock_share_death_timer_expiry_enters_zeroed_wait_slot() -> None:
    import msl_binding
    ...
    seed["action_id"][0, 1] = np.uint16(ACT_DEAD_LEFT)
    seed["match_flow_timer"][0, 1] = np.uint8(1)
    seed["match_flow_pending_rebirth_char_id"][0, 1] = np.uint8(CHAR_FOX)
    seed["pos_x"][0, 1] = np.float32(-250.0)
    seed["pos_y"][0, 1] = np.float32(20.0)
    ...
    assert int(cmp0["char_id"][1]) == 0
    assert int(cmp0["stocks"][1]) == 0
    assert int(cmp0["action_id"][1]) == ACT_DEAD_DOWN
    assert float(cmp0["pos_x"][1]) == 0.0
    assert float(cmp0["shield_hp"][1]) == 0.0
...
def test_zeroed_team_stock_pending_rebirth_restores_fighter_kind_at_timer_expiry() -> None:
    ...
    seed["stocks"][0, 2] = np.uint8(2)
    seed["action_id"][0, 1] = np.uint16(ACT_DEAD_DOWN)
    seed["action_frame"][0, 1] = np.int16(0)
    seed["match_flow_timer"][0, 1] = np.uint8(1)
    seed["match_flow_pending_rebirth_char_id"][0, 1] = np.uint8(CHAR_FOX)
    seed["match_flow_pending_rebirth_state_flags_2218"][0, 1] = np.uint8(0x44)
    ...
    try:
        seed_bytes = seed.view(np.uint8).reshape((1, seed_stride))
        msl_binding.reseed_seed(handle, seed_bytes)
        prev = _input_bytes(1, input_stride)
        inp = _input_bytes(1, input_stride)
        _set_buttons(inp, 1, BUTTON_START)
    ...
    assert int(cmp0["stocks"][2]) == 1
    assert int(cmp0["action_id"][1]) == ACT_REBIRTH
    assert int(cmp0["action_frame"][1]) == 1
    assert int(cmp0["state_flags"][1, 0]) == 0x44
    assert float(cmp0["pos_y"][1]) == pytest.approx(112.830505, abs=0.00001)


def test_zeroed_team_stock_pending_rebirth_waits_without_start_trigger() -> None:
    import msl_binding
    ...
    assert int(cmp0["char_id"][1]) == 0
    assert int(cmp0["stocks"][1]) == 0
    assert int(cmp0["stocks"][2]) == 2
    assert int(cmp0["action_id"][1]) == ACT_DEAD_DOWN


def test_zeroed_team_stock_pending_rebirth_requires_donor_above_one_stock() -> None:
    import msl_binding
