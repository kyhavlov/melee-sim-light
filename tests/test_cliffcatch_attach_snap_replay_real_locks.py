...
def test_cliffcatch_attach_snap_target_pm1_and_negative() -> None:
    # Replay-real lock for CliffCatch attach ownership:
    # - ftCliffCommon_80081370 enters CliffCatch and immediately runs ftCo_CliffCatch_Phys.
    # - ftCo_CliffCatch_Phys keeps CliffCatch snapped to ledge_point + TransNPos while the action
    #   remains in CliffCatch; CliffWait is left to its separate owner.
    ...
    _, ref_prev, out_prev = _run_one_step_row(dataset_path, 2594, p)
    ...
    _, ref_target, out_target = _run_one_step_row(dataset_path, 2595, p)
    _assert_branch_identity_match_ref(out_row=out_target, ref_row=ref_target, p=p)
    assert int(ref_target["action_id"][p]) == 252  # CliffCatch
    ...
    _, ref_next, out_next = _run_one_step_row(dataset_path, 2596, p)
    ...
    assert float(out_next["pos_y"][p]) == pytest.approx(float(ref_next["pos_y"][p]), abs=1e-6)

    _, ref_neg, out_neg = _run_one_step_row(dataset_path, 2599, p)
