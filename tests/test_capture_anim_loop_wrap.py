...
# Fighter part ids (part-index domain in our SSANIM01 v5 blobs).
FTPART_XROTN = 2
...
def _origin_world_facing_yrot90(
    ...
    *,
    ...
    # SSANIM01 v5 matrix record layout: <12f (m00,m01,m02,tx, m10,m11,m12,ty, m20,m21,m22,tz).
    lx = float(m12[3])
    ly = float(m12[7])
    lz = float(m12[11])
...
def test_capture_anim_loop_wrap_applies_before_delta() -> None:
    ...
    mx = fox_pose.try_get_matrix(msid=2, frame=0, part_id=anchor_part)  # Wait1_0 @ frame 0
    mv0 = falco_pose.try_get_matrix(msid=msid_loop, frame=frame_wrapped, part_id=FTPART_XROTN)
    if mx is None or mv0 is None:
        pytest.skip("local anims missing required joint parts for capture loop test; regenerate via extract_fighter_anims")
    ...
    vx0, vy0, vz0 = _origin_world_facing_yrot90(
        mv0,
    ...
    exp_dy = ay - vy0
    exp_dz = az - vz0
    assert np.isfinite([exp_dx, exp_dy, exp_dz]).all()
