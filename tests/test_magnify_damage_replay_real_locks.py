...
def test_magnify_rollout_mgs_damagefly_hi_n_source_visible_episode_ticks() -> None:
    ...
    first_tick = 2919
    first_seed = ds.rows[first_tick]["seed_t"]
    first_ref = ds.rows[first_tick]["ref_t1"]
    ...
    hit_record = 5687
    hit_ref = ds.rows[hit_record]["ref_t1"]
    second_seed = ds.rows[5621]["seed_t"]
...
def test_magnify_rollout_mvp_damageflylw_horizontal_root_exit_starts_counter() -> None:
    ...
    visible_seed = ds.rows[visible_record]["seed_t"]
    assert int(visible_seed["action_id"][p]) == 89  # DamageFlyLw.
    assert int(visible_seed["magnify_damage_counter_x1910"][p]) == 1
    assert int(ds.rows[visible_record - 1]["seed_t"]["magnify_damage_counter_x1910"][p]) == 0
...
def test_magnify_rollout_tvr_damageflyhi_visible_edge_local_episode_ticks() -> None:
    ...
    assert int(ds.rows[visible_record - 1]["seed_t"]["camera_box_visible_x221f_b0"][p]) == 0
...
def test_magnify_rollout_tvr_damageflyn_hitstun_only_visible_edge_repeats_after_tick() -> None:
    ...
    first_ref = ds.rows[first_tick_record]["ref_t1"]
    second_ref = ds.rows[second_tick_record]["ref_t1"]
...
def test_magnify_rollout_tvr_damageflyhi_hitstun_only_visible_edge_ticks() -> None:
    ...
    stale_seed = ds.rows[stale_visible_record]["seed_t"]
    assert int(stale_seed["camera_box_visible_x221f_b0"][p]) == 1
    assert int(ds.rows[stale_visible_record - 1]["seed_t"]["camera_box_visible_x221f_b0"][p]) == 1
...
def test_magnify_rollout_tvr_damageflyroll_visible_edge_carries_through_downstand_tick() -> None:
    ...
    downstream_ref = ds.rows[downstream_record]["ref_t1"]
...
def test_magnify_rollout_lim_damageflyhi_local_episode_resets_when_camera_inside() -> None:
    ...
    source_seed = ds.rows[source_record]["seed_t"]
    ...
    assert int(ds.rows[source_record]["ref_t1"]["state_flags"][p, 4]) & 0x80

    inside_seed = ds.rows[inside_record]["seed_t"]
    assert int(inside_seed["camera_target_point_inside_stage_cam_bounds_u8"][p]) == 1
    assert (int(inside_seed["state_flags"][p, 4]) & 0x80) == 0

    no_tick_ref = ds.rows[no_tick_record]["ref_t1"]
...
def test_magnify_rollout_feh_damageflyhi_no_source_lane_does_not_tick_replay_playback() -> None:
    ...
    land_ref = ds.rows[land_record]["ref_t1"]
...
def test_magnify_rollout_late_damageflytop_b1_visible_edge_does_not_tick() -> None:
    ...
    root = Path(__file__).resolve().parents[1]
    dataset_path = (
        root / "replays/validation/aggregate_recent/Game_20260514T181413.slpz"
    )
    if not dataset_path.exists():
        pytest.skip(f"missing local replay: {dataset_path}")
...
def test_magnify_rollout_dcc_damagefly_top_visible_rows_do_not_start_episode() -> None:
    ...
    visible_top = ds.rows[9362]["seed_t"]
