...
def phase1_binary() -> Path:
    return _repo_root() / "build" / "decomp_port" / "phase1" / "melee-decomp-port"


def phase1_qemu() -> Path:
    return _repo_root() / "build" / "decomp_port" / "ppc-toolchain" / "root" / "usr" / "bin" / "qemu-ppc-static"


def phase1_qemu_sysroot() -> Path:
    return _repo_root() / "build" / "decomp_port" / "ppc-toolchain" / "root" / "usr" / "powerpc-linux-gnu"


def build_phase1(*, jobs: int = 2) -> Path:
    """Build the isolated PPC32 executable without invoking the Python-extension build."""

    root = _repo_root()
    subprocess.run(
        ["make", "-f", "src/decomp_port/Makefile", f"-j{max(1, int(jobs))}", "mvp"],
        cwd=root,
        check=True,
    )
    return phase1_binary()
...
def run_phase1(
    ...
    *,
    ...
    """Run one free-running input tape in one QEMU process and return MslCompare rows."""

    root = _repo_root()
    executable = Path(binary) if binary is not None else phase1_binary()
    qemu = phase1_qemu()
    sysroot = phase1_qemu_sysroot()
    game_data = Path(data_dir) if data_dir is not None else root / "refs" / "melee-disc" / "files"
    if not executable.is_file():
        raise FileNotFoundError(f"Phase 1 binary is missing; run build_phase1(): {executable}")
    if not qemu.is_file():
    ...
    with tempfile.TemporaryDirectory(prefix="msl-decomp-port-") as tmp:
        tmp_dir = Path(tmp)
    ...
    if len(raw) != expected_size:
        raise RuntimeError(
            f"Phase 1 emitted {len(raw)} bytes; expected {expected_size} "
...
def locomotion_tape() -> np.ndarray:
    """Bounded Phase 1 tape covering neutral, ground movement, jump, fall, and landing."""
