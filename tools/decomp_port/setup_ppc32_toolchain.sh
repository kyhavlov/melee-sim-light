...
toolchain_dir="$repo_root/build/decomp_port/ppc-toolchain"
...
for command in apt-get dpkg-deb; do
    if ! command -v "$command" >/dev/null 2>&1; then
        echo "decomp-port toolchain setup requires $command" >&2
        exit 1
    fi
done
...
if [[ ! -x "$sysroot/usr/bin/powerpc-linux-gnu-gcc-13" ||
      ! -x "$sysroot/usr/bin/qemu-ppc-static" ]]; then
    echo "decomp-port PPC32 toolchain extraction was incomplete" >&2
    exit 1
fi
