...
def _evaluate_replay_task(task: dict) -> dict:
    replay_label = Path(str(task["replay_label"]))
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="Parallel replay eval workers (0 = auto, capped at 8).",
    )
    args = ap.parse_args()
    ...
    try:
        ...
        if workers == 1 or len(tasks) <= 1:
            task_results = [_evaluate_replay_task(task) for task in tasks]
        else:
            with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
                task_results = list(executor.map(_evaluate_replay_task, tasks))
