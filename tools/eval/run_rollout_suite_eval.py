...
def _report_header(*, root: Path, suite: str, suite_name: str) -> list[str]:
    ...
    return [
        ...
        "# Validation input: direct .slp/.slpz replay ingest; no validation row cache is read or written.",
        "# Regenerate data artifacts: uv run python -m tools.extraction.build_data (chars default to the registry)",
        "# Rollout triage summaries: make rollout-summary / rollout-diff.",
...
def standard_rollout_fields() -> tuple[str, ...]:
    return ("action_id", "animation_index", "on_ground", "hitlag", "hitstun", "state_flags")
...
def main() -> None:
    ...
    ap.add_argument(
        "--players",
        default=None,
        help="Comma-separated player indices to compare (default: all players in replay).",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="Parallel replay scan workers (0 = auto, capped at 8).",
    )
    ap.add_argument(
        "--exceptions",
        type=Path,
    ...
    ap.add_argument(
        ...
        type=int,
        default=3,
        help="Top-N rollout float-only and downstream rows to display per replay (0 disables).",
    )
    args = ap.parse_args()
