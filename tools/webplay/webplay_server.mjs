...
const sourcePath = path.join(__dirname, "gcadapter_stream.c");
const binaryPath = path.join(repoRoot, "reports/triage/gcadapter_stream");
const port = Number(process.env.MSL_WEBPLAY_PORT || 8001);
const host = process.env.MSL_WEBPLAY_HOST || "127.0.0.1";
const shouldOpenBrowser = process.env.MSL_WEBPLAY_OPEN !== "0";
...
const allowedStaticPrefixes = [
  "/tools/webplay/",
  "/tools/modelplay/viewer/dist/",
  "/tools/modelplay/viewer/public/",
];

function checkAssets() {
  const required = [
    "tools/webplay/index.html",
    "tools/webplay/public/msl_sim.js",
    "tools/webplay/public/msl_sim.wasm",
    "tools/webplay/public/msl_sim.data",
    "tools/modelplay/viewer/dist/index.js",
  ];
  const missing = required.filter((rel) => !fs.existsSync(path.join(repoRoot, rel)));
  if (missing.length > 0) {
    console.error("Missing webplay assets:");
    for (const rel of missing) {
      console.error(`  ${rel}`);
    }
    console.error("Build them with:");
    console.error("  tools/webplay/build_wasm.sh");
    console.error("  npm --prefix tools/modelplay/viewer run build");
    process.exit(1);
  }
}
...
server.on("error", (error) => {
  if (error.code === "EADDRINUSE") {
    console.error(`Webplay server could not bind ${host}:${port}: address already in use.`);
    console.error("Stop the existing server or choose another port with WEBPLAY_PORT=... / make WEBPLAY_PORT=...");
    if (streamProcess) {
...
server.listen(port, host, () => {
  ...
  const url = `http://${browserHost}:${port}/tools/webplay/`;
  const listenUrl = `http://${host}:${port}/tools/webplay/`;
  console.log(`Webplay: ${url}`);
  if (listenUrl !== url) {
