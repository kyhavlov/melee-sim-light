#!/usr/bin/env python3
"""Generate native DAT field descriptions from PPC metadata and host DWARF.

The decomp declarations are the schema.  The PPC32 layout is generated by
make ppc-layout and versioned; the host layout is compiled during each build.
...
class Dwarf:
    ...
    @classmethod
    def from_layout(cls, path: Path) -> "Dwarf":
        layout = json.loads(path.read_text())
        if layout["version"] != 1 or layout["address_size"] != 4:
            raise ValueError(f"unsupported PPC layout: {path}")
        dwarf = cls.__new__(cls)
        dwarf.address_size = layout["address_size"]
        dwarf.dies = {}
        for index, node in enumerate(layout["dies"]):
            attrs = {key: str(value) for key, value in node["attrs"].items()}
            if "DW_AT_type" in attrs:
                attrs["DW_AT_type"] = f"<0x{int(attrs['DW_AT_type']):x}>"
            dwarf.dies[index] = Die(index, 0, node["tag"], attrs)
        for index, node in enumerate(layout["dies"]):
            dwarf.dies[index].children = [dwarf.dies[i] for i in node["children"]]
        return dwarf
...
def main() -> None:
    ...
    src = Dwarf.from_layout(args.ppc)
    dst = Dwarf(args.native)
    builder = PairBuilder(src, dst)
    roots = [
        (name, builder.pair(src.named(name), dst.named(name))) for name in DAT_ROOT_NAMES
    ]
