...
- `misc0`, `misc1`, `misc2`, `misc3`: item-type-specific small values, the
  ...
  (Samus and Mewtwo). Readers treat a missing field as `0`, so older traces
  without `misc3` remain valid.
...
- `hitboxX`, `hitboxY`, `hitboxRadius`: the item's first live hit capsule
  (Fox Illusion, Yoshi's stars, PK Fire pillar, Blizzard); `hitboxRadius` is
  `0` when the item has no active hitbox.
- `tipValid`, `tipX`, `tipY`: far end of a tether's link chain (hookshot,
  grapple beam), which is the ledge grab point while hanging. Prefer promoting these to named fields when
  they become stable viewer/debug concepts.
