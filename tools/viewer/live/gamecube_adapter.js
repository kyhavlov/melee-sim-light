...
export class GameCubeAdapterInput {
  ...
  async #connectWebHid() {
    ...
    if (!device) {
      ...
      try {
        ...
        if (error.name === "NotFoundError") {
          throw new Error(
            "No compatible HID devices found. On Linux, make sure the adapter has a hidraw node; run tools/viewer/live/linux_webhid_setup.sh to diagnose."
          );
        }
        throw error;
      }
    ...
    if (!device) {
      throw new Error(
        "No GameCube adapter selected. On Linux, make sure the adapter has a hidraw node; run tools/viewer/live/linux_webhid_setup.sh to diagnose."
      );
    }
