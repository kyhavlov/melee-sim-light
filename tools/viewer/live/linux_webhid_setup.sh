...
The viewer's live-play path uses the local libusb bridge, which needs raw USB
access and usually works best with no kernel driver bound. The older WebHID
experiment requires a /dev/hidraw* node, which needs the usbhid driver.
EOF
    ;;
  "")
    print_status
