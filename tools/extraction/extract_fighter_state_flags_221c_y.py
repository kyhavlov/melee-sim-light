...
def _build_flags_by_frame(events: list[object], *, max_frames: int) -> list[int]:
    # Decomp: ftAction_80072C6C writes fp->x221C_u16_y via ft_8008A1B8(flags).
    ...
    for frame in range(max_frames):
        for ev in events_by_frame[frame]:
            if getattr(ev, "kind", None) != "set_state_flags_221c_u16_y":
                continue
            data = getattr(ev, "data", None)
            try:
                flags = int((data or {}).get("flags", 0))
            except Exception:
                flags = 0
            # Debug printer in dbanim shows 3 one-bit lanes: L/R/T (1,2,4).
...
def _write_bin(out_path: Path, *, msids: list[int], payloads_by_msid: dict[int, list[int]], max_frames: int) -> None:
    ...
    # - payload: concatenated u8[frame_count] flags (low 3 bits used)
    ...
    with out_path.open("wb") as f:
        ...
        for msid in msids:
            ...
            for fl in frames:
                f.write(struct.pack("<B", int(fl) & 0xFF))


def main() -> None:
    ap = argparse.ArgumentParser(
        description="Extract movescript-derived fp->x221C_u16_y timelines (opcode 52) into compact .bin tables."
    )
    ...
    ap.add_argument("--out_dir", type=Path, default=Path("data/state_flags_221c_y"))
    ...
    for ch in [c.strip() for c in args.chars.split(",") if c.strip()]:
        ...
        domain = sorted(set(msid_candidates) | set(_load_special_msids(args.special_msids_dir, ch)))
        for msid in domain:
            ...
            timeline = _build_flags_by_frame(events, max_frames=int(args.max_frames))
            if any(int(x) != 0 for x in timeline):
