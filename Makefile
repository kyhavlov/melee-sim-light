...
VALIDATION_CHARACTERS ?= Mewtwo,Game & Watch,Fox,Falco,Marth,Captain Falcon,Sheik,Zelda,Jigglypuff,Peach,Luigi,Mario,Dr. Mario,Samus,Ice Climbers,Pikachu,Donkey Kong,Ganondorf,Yoshi,Bowser,Ness,Link,Young Link
...
gamewatch-smoke: data-check $(NATIVE_BUILD)/gamewatch-moves-smoke
	@$(TIMEOUT) 10s "$(NATIVE_BUILD)/gamewatch-moves-smoke" "$(DATA)"
.PHONY: fnmsubs-smoke
fnmsubs-smoke: $(NATIVE_BUILD)/fnmsubs-smoke
	@$(TIMEOUT) 10s "$(NATIVE_BUILD)/fnmsubs-smoke"
