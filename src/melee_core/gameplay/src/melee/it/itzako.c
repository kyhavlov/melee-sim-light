                       ...
                       bool use_init)
{
    ...
    if (gobj != NULL) {
        ...
        if (kind == It_Kind_Heiho) {
            // Retail aliases Zako's 32-bit pointer/index words with Heiho's
            // first two f32 slots. Preserve those PPC source offsets when the
            // host HSD_JObj pointer widens.
            // refs/melee/src/melee/it/{itCommonItems.h::itZako_ItemVars,
            // items/itheiho.c::it_802D8618}
            uint32_t source_jobj = (uint32_t) (uintptr_t) jobj;
            int32_t source_idx = -1;
            memcpy((uint8_t*) &ip->xDD4_itemVar + 0x00, &source_jobj,
                   sizeof(source_jobj));
            memcpy((uint8_t*) &ip->xDD4_itemVar + 0x04, &source_idx,
                   sizeof(source_idx));
            memset((uint8_t*) &ip->xDD4_itemVar + 0x1C, 0, sizeof(int32_t));
        } else
#endif
        {
...
#ifdef MSL_CORE_HOSTED
    if (item->kind == It_Kind_Heiho) {
        uint32_t source_jobj = 0;
        int32_t source_idx = -1;
        Vec3 source_x8 = { 0.0F, 1.0F, 0.0F };
        S32Vec3 source_x14 = { 0, 0, 0 };
        uint8_t* vars = (uint8_t*) &item->xDD4_itemVar;
        memcpy(vars + 0x00, &source_jobj, sizeof(source_jobj));
        memcpy(vars + 0x04, &source_idx, sizeof(source_idx));
        memcpy(vars + 0x08, &source_x8, sizeof(source_x8));
        memcpy(vars + 0x14, &source_x14, sizeof(source_x14));
    } else
#endif
    {
