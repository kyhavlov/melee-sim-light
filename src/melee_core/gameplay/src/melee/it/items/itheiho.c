...
typedef struct MslHeihoAttrs {
    s32* damage_threshold;
    f32 values[6];
} MslHeihoAttrs;
#endif

static inline f32 itHeiho_GetSpecialFloat(Item* ip, int source_index)
{
#ifdef MSL_CORE_HOSTED
    MslHeihoAttrs* attrs = ip->xC4_article_data->x4_specialAttributes;
    return attrs->values[source_index - 1];
#else
    f32* attrs = ip->xC4_article_data->x4_specialAttributes;
    return attrs[source_index];
#endif
}

static inline s32 itHeiho_GetDamageThreshold(Item* ip)
{
#ifdef MSL_CORE_HOSTED
    MslHeihoAttrs* attrs = ip->xC4_article_data->x4_specialAttributes;
    return *attrs->damage_threshold;
#else
...
void itHeiho_UnkMotion1_Phys(Item_GObj* gobj)
{
    ...
    ip->x40_vel.x =
        ip->facing_dir *
        itHeiho_GetSpecialFloat(ip, ip->xDD4_itemVar.heiho.x21 + 1);
...
static void it_802D8EC8_inline(Item_GObj* gobj)
{
    ...
    ip->x40_vel.x += itHeiho_GetSpecialFloat(ip, 5) * rand;
    ip->x40_vel.y = 2.0F;
    ip->x40_vel.z = 1.5f;
    if (ip->facing_dir == -1.0F) {
        HSD_JObjSetRotationY(HSD_GObjGetHSDObj(gobj), 3.1415927F);
...
bool it_802D8EC8(Item_GObj* gobj)
{
    ...
    ip->xDCC_flag.b3 = 1;
    if (ip->xC9C > (s32) (itHeiho_GetDamageThreshold(ip) * 0.8F)) {
        it_8027CE44(gobj);
...
#endif
    // The omitted TObj AObj changes texture playback only. The child JObj
    // AObj remains gameplay-bearing because it_802D98C4 turns that animated
    // bone translation into the Shy Guy's velocity on the next frame.
    // refs/melee/src/melee/it/items/itheiho.c::itHeiho_UnkMotion4_Anim
    return 0;
}
