...
void grIzumi_OnLoad(void)
{
    ...
    // Lighting animation is render-only in the headless runtime.
    // refs/melee/src/melee/gr/grizumi.c::grIzumi_OnLoad
    return;
#else
    HSD_GObj* gobj;
    HSD_LObj* lobj;
...
void grIzumi_801CBE64(Ground_GObj* gobj)
{
...
#else
    // The EFB reflection graph is presentation-only. The moving-platform
    // JObjs and collision callbacks below remain the gameplay owner.
    // refs/melee/src/melee/gr/grizumi.c::grIzumi_801CBE64
    gp->gv.izumi.xC4 = NULL;
    gp->gv.izumi.xC8 = NULL;
    gp->gv.izumi.xCC = NULL;
#endif
#ifndef MSL_CORE_HOSTED
    ...
    { // this looks like inlines, but there's a lot of small differences
        ...
        lb_8000B1CC(jobj, NULL, &x38);
        {
            ...
            // The source writes gp+0xDC through the izumi2 union view. On a
            // 64-bit host its leading pointers widen, so name the actual
            // moving-platform (izumi3) owner and preserve that source offset.
            // refs/melee/src/melee/gr/grizumi.c::grIzumi_801CBE64
            platground->gv.izumi3.xDC = grIz_804D6968->xC;
                                               ...
                                               bool changed)
{
    Ground* gp = GET_GROUND(gobj);
    HSD_JObj* jobj = GET_JOBJ(gobj);
    f32 collision_height = height;

    gp->gv.izumi3.xD0 = height;
    if (!changed && height <= -1.0F) {
        // At the hidden target, case 2 publishes xD0 once (and Slippi records
        // it), then case 3 performs an unrecorded source publication using
        // the actor JObj's base translation minus one. Preserve that second
        // publication while the replay event stream is quiescent.
        // refs/melee/src/melee/gr/grizumi.c::grIzumi_801CC358 cases 2..4
        HSD_JObjSetFlagsAll(jobj, JOBJ_HIDDEN);
        HSD_JObjRemoveAnimAll(jobj);
        collision_height = HSD_JObjGetTranslationY(jobj) - 1.0F;
    } else {
        HSD_JObjClearFlagsAll(jobj, JOBJ_HIDDEN);
    }
    HSD_JObjSetTranslateY(gp->gv.izumi3.xCC, collision_height);
    mpLib_80055E9C(gp->gv.izumi3.xC8);
}
#endif
