...
static inline s32 grZakoGenerator_GetSourceIndex(Item* ip)
{
#ifdef MSL_CORE_HOSTED
    s32 result;
    // itZako_ItemVars::idx follows a retail 32-bit pointer. Read its source
    // union offset so widened host pointers cannot move generator ownership.
    // refs/melee/src/melee/it/itCommonItems.h::itZako_ItemVars
    memcpy(&result, (uint8_t*) &ip->xDD4_itemVar + 0x04, sizeof(result));
    return result;
#else
    return ip->xDD4_itemVar.zako.idx;
#endif
}
...
void grZakoGenerator_801CAC14(HSD_GObj* gobj)
{
    ...
    if (kind == 0x9F) {
        ...
        s32 idx = grZakoGenerator_GetSourceIndex(ip);
        if (idx != -1) {
