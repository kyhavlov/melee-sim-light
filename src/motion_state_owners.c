...
enum {
  TABLE_MAGIC_LEN = 8,
  TABLE_HDR_BYTES = 8 + 4 + 2 + 2 + 4 * 15,
};

static const uint8_t k_magic[TABLE_MAGIC_LEN] = {'M', 'S', 'L', 'M', 'S', 'O', '0', '1'};
static const uint32_t k_format_version = 28;
...
typedef struct {
  ...
  uint8_t* coll_wrapper_selector_kind_by_action;
  uint8_t* coll_source_plan_by_action;
...
static int load_table_for_char_into(uint8_t char_id, const char* rel_name,
  ...
  const uint32_t coll_wrapper_selector_kind_off = read_u32_le(buf + 68);
  const uint32_t coll_source_plan_off = read_u32_le(buf + 72);
  const uint64_t file_bytes = (uint64_t)coll_source_plan_off + (uint64_t)action_count * 4u;
  ...
  if (submotion_off < (uint32_t)TABLE_HDR_BYTES || x4_flags_off < (uint32_t)TABLE_HDR_BYTES ||
      ...
      coll_handler_kind_off < (uint32_t)TABLE_HDR_BYTES ||
      coll_wrapper_selector_kind_off < (uint32_t)TABLE_HDR_BYTES ||
      coll_source_plan_off < (uint32_t)TABLE_HDR_BYTES || !range_ok(x4_flags_off, u32_bytes, sz) ||
      ...
      !range_ok(coll_wrapper_selector_kind_off, (uint32_t)action_count, sz) ||
      !range_ok(coll_source_plan_off, u32_bytes, sz)) {
  ...
  out->coll_wrapper_selector_kind_by_action = buf + coll_wrapper_selector_kind_off;
  out->coll_source_plan_by_action = buf + coll_source_plan_off;
...
uint8_t msl_motion_state_coll_handler_kind(uint8_t char_id, uint16_t action_id) {
  ...
  return in_range(t, action_id) ? t->coll_handler_kind_by_action[action_id]
                                : (uint8_t)MSL_COLL_HANDLER_NONE;
}

uint8_t msl_motion_state_coll_wrapper_selector_kind(uint8_t char_id, uint16_t action_id) {
  const MslMotionStateOwnerTable* t = table_for_char(char_id);
  return in_range(t, action_id) ? t->coll_wrapper_selector_kind_by_action[action_id]
                                : (uint8_t)MSL_COLL_SELECTOR_NONE;
}

uint32_t msl_motion_state_coll_source_plan(uint8_t char_id, uint16_t action_id) {
  const MslMotionStateOwnerTable* t = table_for_char(char_id);
  return in_range(t, action_id)
             ? read_u32_le(t->coll_source_plan_by_action + (size_t)action_id * 4u)
             : 0u;
}
