...
typedef struct MslCombatBodyDamageProducer {
  uint16_t move_id;
  uint16_t attack_instance;
  float applied_damage;
  int hitcapsule_int_dmg;
  int env_dmg;
} MslCombatBodyDamageProducer;
...
static inline uint8_t combat_shield_overlap_ftcoll_80007bcc(
  ...
  if (guardon_raise_shield_no_submotion_attackair_x10) {
    ...
    if (msl_shield_tilt_table_view(batch->state.char_id[d_idx], &tv) == 0 && tv.xyz != NULL &&
      ...
      //   (animation_index=-1/action_frame=-1).
      // - This retained runtime owner is the aerial fighter HitCapsule path, using the generated
      //   MSLMSO01 AttackAir class rather than local action ids. Shine/projectile shield
      //   acceptance remains on the shared shield bubble owner until its separate source geometry
  ...
  const uint8_t attackairlw_fresh_guardon_shielddesc_size_lane =
      (guardon_already_shielding_no_submotion && batch->state.hitbox_enable_edge[hb_i] &&
       ...
       batch->state.animation_index[a_idx] == (uint32_t)MSL_SM_ATTACK_AIR_LW)
          ? 1u
          : 0u;
  // GuardOn no-submotion snapshots can last beyond the actual ftCo_800924C0 entry callback. Keep
  // the extra ShieldDesc.size term on the source-owned create/copy/clear edge only; persistent
  // HitCapsules must use the ordinary shield matrix radius or late AttackAirB sweep rows over-admit
  // GuardSetOff after vanilla has proven a ShieldDesc miss.
  ...
  // after ftAction_8007121C/ftColl_8007AD18 ownership. The retained fresh AttackAirLw/GuardOn lane
  // above keeps the ShieldDesc.size term passed directly to lbColl_80006E58; the reduced 0.2 extent
  // bridge remains only for steady capsules where this sim's shield proxy otherwise over-accepts
  // broadphase-only grazes. Already-shielding no-submotion GuardOn rows still suppress the extra
  // broadphase extent lane. Continuing GuardOn x20 rows only receive that reduced extent when the
  // GuardOn raise-shield owner is on the no-tilt x20 target; angled rows already blend through the
  // generated tilt table above and replay-real doubles rows prove the reduced proxy over-accepts
  // their tilted ShieldDesc matrix.
  // refs/melee/src/melee/ft/ftaction.c::ftAction_8007121C
  // refs/melee/src/melee/ft/ftcoll.c::{ftColl_8007AD18,ftColl_80078C70}
  // refs/melee/src/melee/lb/lbcollision.c::{lbColl_80007BCC,lbColl_80006E58}
...
static inline uint8_t combat_body_damage_producer_build(const MslBatch* batch, size_t a_idx,
                                                        ...
                                                        uint16_t attacker_attack_id,
                                                        MslCombatBodyDamageProducer* out) {
  ...
  out->move_id = attacker_attack_id;
  out->attack_instance = batch->state.attack_instance[a_idx];

  float hb_dmg = combat_apply_attacker_smash_release_damage_mul(batch, a_idx,
                                                                batch->state.hitbox_damage[hb_i]);
  out->hitcapsule_int_dmg = (batch->state.smash_charge_state[a_idx] == 3u) ? (int)hb_dmg : int_dmg;

  const float stale_mult = staling_multiplier_for_move(batch, a_idx, out->move_id);
  if (stale_mult != 1.0f) {
    hb_dmg *= stale_mult;
  }
  out->applied_damage = hb_dmg;
  out->env_dmg = combat_get_env_dmg(hb_dmg);
  return (out->env_dmg > 0) ? 1u : 0u;
}

static inline void combat_body_damage_producer_apply_attacker_side(
    MslBatch* batch, size_t a_idx, const MslCombatBodyDamageProducer* prod,
    uint16_t attacker_motion_id) {
...
static inline void combat_body_damage_log_entry_init(
    ...
    const MslCombatBodyDamageProducer* prod) {
...
static inline void combat_mutations_pass1_future_apply_body_hit(
  ...
  combat_damage_enter_state(c, batch, (int)bi, d_idx, defender_on_ground, defender_on_ground_after,
                            hurt_height, kb_applied, kb_angle_rad, hb_angle);
...
static inline uint8_t combat_body_damage_log_record(
  ...
  MslCombatBodyDamageProducer prod;
