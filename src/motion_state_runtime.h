...
// Compatibility boundary for action owners that still write action_id directly. It installs only
// on an action change; persistent source callback overrides are not reconstructed every frame.
...
// `handler_kind` is the stable transition owner and `source_plan` is the callback's exact mpColl
// wrapper recipe. They must change atomically with the diagnostic pointer identity.
