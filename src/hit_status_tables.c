...
static const uint8_t k_magic[HITSTATUS_MAGIC_LEN] = {'M', 'S', 'L', 'H', 'S', 'T', 'A', '1'};
...
typedef struct {
  uint8_t* status;  // [entry_count * frame_count]
  uint32_t status_count;

  uint8_t* have_msid;            // [65536]
  uint32_t* base_index_by_msid;  // [65536] (index into status[])
...
static MslHitStatusTable g_table_by_char[256];
...
static void free_table(MslHitStatusTable* t) {
  if (t == NULL) {
    return;
  }
  alloc_free(t->status);
...
static int load_for_char(const char* data_dir, const char* rel_path, uint8_t char_id) {
  ...
  if (sz < HITSTATUS_HDR_BYTES) {
    alloc_free(buf);
    return -1;
  }
  if (memcmp(buf, k_magic, HITSTATUS_MAGIC_LEN) != 0) {
    alloc_free(buf);
    return -1;
  }
  const uint32_t ver = read_u32_le(buf + 8);
  if (ver != (uint32_t)HITSTATUS_VERSION_V1) {
    alloc_free(buf);
    return -1;
  }
  ...
  const size_t index_end = (size_t)HITSTATUS_HDR_BYTES + index_bytes;
  if (index_end > sz) {
    alloc_free(buf);
    return -1;
  }
  ...
  for (uint32_t i = 0; i < entry_count; i++) {
    const size_t off = (size_t)HITSTATUS_HDR_BYTES + (size_t)i * (size_t)INDEX_REC_BYTES_V1;
  ...
  if (have_msid == NULL || base_index_by_msid == NULL || status == NULL) {
  ...
  for (uint32_t i = 0; i < entry_count; i++) {
    ...
    memcpy(status + base, buf + (size_t)payload_off, (size_t)frame_count);
  }
  ...
  g_table_by_char[char_id] = (MslHitStatusTable){
      .status = status,
      .status_count = (uint32_t)entry_count * (uint32_t)frame_count,
      .have_msid = have_msid,
...
int hit_status_tables_init(void) {
  ...
  if (load_for_char(data_dir, "hit_status/fox.bin", MSL_CHAR_FOX) != 0) {
    return -1;
  }
  if (load_for_char(data_dir, "hit_status/falco.bin", MSL_CHAR_FALCO) != 0) {
...
int hit_status_get(uint8_t char_id, uint16_t msid, uint16_t frame, uint8_t* out_status) {
  ...
  *out_status = 0;
  ...
  const MslHitStatusTable* t = &g_table_by_char[char_id];
  if (!t->have || t->status == NULL || t->have_msid == NULL || t->base_index_by_msid == NULL ||
      t->frame_count == 0) {
    return 0;
  }
  ...
  if ((uint64_t)base + (uint64_t)f >= (uint64_t)t->status_count) {
    return 0;
  }
