...
static inline uint8_t mpcoll_non_air_common_damage_submotion(uint32_t smid) {
  switch (smid) {
...
static inline uint16_t mpcoll_probe_segment_for_line(const MslStageFloorGraph* g, int line_idx) {
...
static inline void mpcoll_floor_probe_clear(MslBatch* batch, size_t idx) {
  if (batch == NULL) {
    return;
  }
...
static inline void mpcoll_floor_probe_begin(const MslMpcollContext* ctx, uint8_t owner,
                                            MslMpcollSourcePhases source_phases,
                                            int candidate_line_idx, uint8_t reject_reason) {
...
static inline void mpcoll_floor_probe_lines(const MslMpcollContext* ctx, int candidate_line_idx,
                                            int projected_line_idx) {
  if (ctx == NULL || ctx->batch == NULL) {
    return;
  }
...
static inline void mpcoll_floor_probe_reject_bits(const MslMpcollContext* ctx, uint64_t bits,
                                                  MslMpcollSourcePhases source_phases,
                                                  int candidate_line_idx, int projected_line_idx) {
  if (ctx == NULL || ctx->batch == NULL) {
    return;
  }
  MslBatch* batch = ctx->batch;
...
static inline void mpcoll_floor_probe_bottom_interval(const MslMpcollContext* ctx,
...
static inline void mpcoll_floor_probe_result(const MslMpcollContext* ctx,
                                             const MslMpcollFloorSweepResult* result,
                                             uint8_t raw_hit, uint8_t projection_hit,
...
static inline void mpcoll_record_escapeair_floor_producer_runtime_authority(
...
static inline uint8_t mpcoll_floor_sweep_prev_root_is_runtime_owned(const MslBatch* batch,
                                                                    size_t idx) {
...
static inline uint8_t floor_line_normal_for_env(const MslBatch* batch, int bi,
                                                const MslStageFloorGraph* g, int line_idx,
...
static inline uint8_t specialhi_floor_candidate_starts_below_source_floor(
    uint8_t char_id, uint16_t action_id, float prev_root_y, float prev_bottom_y, float cur_bottom_y,
    float floor_y, float speed_y_self) {
  ...
  const uint8_t generated_slope_owner =
      (floor_line_is_generated_stage_slope(batch, bi, g, current_line_idx) ||
       floor_line_is_generated_stage_slope(batch, bi, g, projected_line_idx))
          ? 1u
          : 0u;
  if (action_id == (uint16_t)MSL_ACT_LANDING_FALL_SPECIAL && action_frame == 0u &&
      generated_slope_owner) {
    // LandingFallSpecial entry on generated legal-stage slopes:
    // `ftCo_LandingFallSpecial_Enter -> ftCo_Landing_Enter` first calls ftCommon_8007D7FC, then the
    // grounded collision callback keeps the current CollData floor attached through the same
    // source floor. Apply signed DD90 correction on the entry row when the floor graph already
    // proves the generated-slope owner; ordinary Landing/LandingAir entry rows keep the root/ECB
    // handoff guard below.
...
static uint8_t msl_mpcoll_80047e14_common_air_hard_floor_bottom_sweep(
  ...
  if (probe_this_47e14_helper) {
    const MslMpcollSourcePhases helper_source_phases =
        ctx != NULL ? mpcoll_source_phases_for_motion_state(ctx->char_id, action_id) : 0u;
...
static uint8_t msl_mpcoll_800473cc_damage_stay_airborne_hard_floor_sweep(
...
static uint8_t msl_mpcoll_80047e14_fallspecial_connected_hard_floor_root_projection(
    MslBatch* batch, size_t idx, int bi, const MslStageFloorGraph* g, uint32_t stage_id,
  ...
  // connected hard-floor graph even when the loaded FallSpecial ECB bottom is still above the
  // floor. Keep the reconstruction to the shallow later-sustained non-fastfall callback window:
  // the preceding IPW callback and frame-start fastfall rows remain airborne, and far-below
  // offstage FallSpecial rows must not snap back to a stale carried ledge. Fastfall/bottom-sweep
  // rows remain on `mpColl_80044628_Floor`; platforms and transformed floors stay on the
  // callback-admitted sweep owner.
  //
  // data/stages/bin/*.bin::MSLSTG01 floor prev/next/fighter_solid/platform_transform metadata
  // refs/melee/src/melee/ft/chara/ftCommon/ftCo_FallSpecial.c::{
  //   ftCo_FallSpecial_Coll,ftCo_80096CC8,ftCo_80096D28}
  // refs/melee/src/melee/ft/ft_081B.c::ft_80083090
  // refs/melee/src/melee/mp/mpcoll.c::{mpColl_80047E14,mpColl_80044628_Floor,
