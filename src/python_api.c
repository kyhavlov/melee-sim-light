...
int msl_python_controller_inputs(const MslPythonControllerInput* source, size_t source_stride,
                                 MslInput* output, size_t output_stride, uint32_t count) {
  uint32_t row;
  if (source == NULL || output == NULL || source_stride < sizeof(*source) ||
...
_Static_assert(sizeof(MslPythonControllerPlayer) == 28, "Python controller player wire size");
_Static_assert(sizeof(MslPythonControllerInput) == 112, "Python controller input wire size");
