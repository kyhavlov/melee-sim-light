...
// it_802B75FC recomputes the x34..x58 attribute lanes in place at every
...
itSamusGrappleAttributes* msl_samus_grapple_attrs(Item* ip)
{
    ...
    if (!state->samus_grapple.seeded) {
        state->samus_grapple.attrs =
            *(itSamusGrappleAttributes*)
                 ip->xC4_article_data->x4_specialAttributes;
        state->samus_grapple.seeded = 1;
    }
    return &state->samus_grapple.attrs;
}
#define MSL_SAMUS_GRAPPLE_ATTRS(ip) msl_samus_grapple_attrs(ip)
#else
#define MSL_SAMUS_GRAPPLE_ATTRS(ip) \
...
void it_802B7160(Fighter_GObj* gobj, itSamusGrapple_HitboxData* data)
{
    ...
    // Hosted compilers do not all honor the big-endian storage pragma, so
    // read the authored field instead of the retail (u16*)((u8*)data + 2)
    // aliasing view of the same damage bits.
...
HSD_JObj* it_802B75FC(Item* ip, HSD_JObj* jobj_arg, s32 arg2, f32 scale)
{
    ...
    for (i = 0; i < attrs->x34; i++) {
        ...
        if (i == 0) {
            ...
            attrs2 = MSL_SAMUS_GRAPPLE_ATTRS(ip);
...
Item_GObj* it_802B7C18(Fighter_GObj* owner, Vec3* pos, f32 facing_dir)
{
    ...
    if (item_gobj != NULL) {
        ...
        attrs = MSL_SAMUS_GRAPPLE_ATTRS(ip);
                 ...
                 itSamusGrappleAttributes* attrs, f32 target_dist)
{
    ...
    while (link != NULL && link->x2C_b0) {
        ...
        if (it_802A3C98(&link->pos, &head->pos, &dir) > attrs->x38) {
            // The upstream NonMatching TU reads dir_ptr before this
            // assignment; MWCC's codegen happened to hold &dir in the
