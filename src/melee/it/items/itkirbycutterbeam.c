...
void itKirbycutterbeam_UnkMotion0_Phys(Item_GObj* gobj)
{
    ...
    if (ip->ground_or_air == GA_Ground) {
        if (ip->x40_vel.y < 0.0f) {
            // GALE01 0x8029BD54 folds this ground damping into one fmadds;
            // the separate multiply and add double-round the pre-snap y
            // (observed as the init_pos.y low byte on a sliding beam).
            ip->x40_vel.y = __fmadds(0.05f, ip->x40_vel.y, ip->x40_vel.y);
        }
    }
    ip->x40_vel.z = 0.0f;
