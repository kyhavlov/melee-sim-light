...
void itLinkboomerang_UnkMotion1_Phys(Item_GObj* gobj)
{
    ...
    if (ip->xDD4_itemVar.linkboomerang.xDE8 != 1) {
        ...
        // Retail stores the attribute back to itself; the hosted native DAT
        // arena is sealed read-only after initialization, so skip only this
        // value-identical self-store.
        attrs->xC = attrs->xC;
#endif
