...
#define HSD_JObjAnim(joint) msl_fighter_pose_animate_joint(joint)
#define HSD_JObjAnimAll(root) msl_fighter_pose_animate_tree(root)
#define HSD_JObjReqAnimByFlags(joint, flags, frame)                           \
    msl_fighter_pose_request_joint((joint), (frame))
#define lb_8000B074(joint) msl_fighter_pose_is_animating(joint)
#define lbGetJObjFramerate(root) msl_fighter_pose_tree_rate(root)
#define lbGetJObjCurrFrame(root) msl_fighter_pose_tree_frame(root)
#define lbGetJObjEndFrame(root) msl_fighter_pose_tree_end(root)
#define ftAnim_Foreach_HSD_AObjSetFlags(root, value)                         \
    msl_fighter_pose_set_tree_flags((root), (value))
#define ftAnim_Foreach_HSD_AObjSetRate(root, value)                          \
    msl_fighter_pose_set_tree_rate((root), (value))
#define HSD_ForeachAnim(root, type, mask, function, argument_type, value)    \
    ftAnim_Foreach_##function((root), (value))
#endif
#else
                     ...
                     HSD_JObj* arg3)
{
    ...
    while (jobj != NULL) {
        ...
        } else if (Fighter_AnimFlag(fp, 5) && jobj == arg3) {
            if (jobj != NULL) {
...
{
    struct Fighter_WaitAnimData* temp_ret;
    ...
    if (Fighter_WaitAnimFlag(temp_ret, 1)) {
...
#define ftAnim_AttachAll(jobj, tree, track, frames)                           \
    msl_fighter_pose_attach_figa((jobj), (tree), (track), (frames), false)
#define ftAnim_AttachFiltered(jobj, tree, track, frames)                      \
    msl_fighter_pose_attach_figa((jobj), (tree), (track), (frames), true)
#else
#define ftAnim_AttachAll(jobj, tree, track, frames)                           \
...
void ftAnim_8006F4C8(Fighter* fp, bool do_blending, FigaTree* tree)
{
    ...
    flags = Fighter_AnimSourceParts(fp);
...
void ftAnim_8006F628(Fighter* fp, Fighter_Part part, bool do_blending)
{
    ...
    tree = fp->x590;
    cur_node = tree->nodes;
    x594_bits = Fighter_AnimSourceParts(fp);
    cur_track = tree->tracks;
    kind = Fighter_AnimSourceKind(fp);
...
void ftAnim_8006F7C8(Fighter* ft, Fighter_Part part, int arg2, FigaTree* tree)
{
    ...
    nodes = tree->nodes;
    tracks = tree->tracks;
    r29 = Fighter_AnimSourceParts(ft);
...
void ftAnim_8006F954(Fighter* fp, Fighter_Part part, bool do_blending,
                     FigaTree* unused)
{
    if (fp->kind != Fighter_AnimSourceKind(fp)) {
...
{
#ifdef MSL_CORE_HOSTED
    // Texture animation only mutates renderer-owned TObj/AObj state. Action
    // script advancement and the fighter's x221E_b7 lifetime remain owned by
    // ftAction_800726F4, ftAnim_800704F0, and ftAnim_80070654.
    // refs/melee/src/melee/ft/ftanim.c::{ftAnim_80070458,ftAnim_800704F0}
    (void) fp;
    (void) tobj_list;
    (void) tobj_idx;
    (void) frame;
...
{
#ifdef MSL_CORE_HOSTED
    // Paired renderer-only reset for the hosted omission above.
    // refs/melee/src/melee/ft/ftanim.c::ftAnim_800705E0
    (void) tobj_list;
#else
    int i;
...
{
#ifdef MSL_CORE_NATIVE
    msl_fighter_pose_request_tree(jobj, frame);
#else
...
{
#ifdef MSL_CORE_NATIVE
    msl_fighter_pose_request_joint(jobj, frame);
#else
...
{
#ifdef MSL_CORE_NATIVE
    msl_fighter_pose_remove_tree(jobj);
#else
