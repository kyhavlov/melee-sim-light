...
void ftLg_SpecialS_Anim(HSD_GObj* gobj)
{
        ...
        fp->x914[0].state == HitCapsule_Enabled)
    {
        ftColl_8007ABD0(&fp->x914[0],
                        // Retail keeps charge*slope+tilt fused.
                        // GALE01 0x801432B8/0x80143364/0x80143738/0x801437E4
                        __fmadds(
...
void ftLg_SpecialSFly_Enter(HSD_GObj* gobj)
{
    ...
    if (fp->mv.lg.SpecialS.isMisfire) {
        ...
        // Retail GALE01 0x80143CC0 fuses mul_x*charge+vel_x.
        fp->self_vel.x = __fmadds(sa->x28_LUIGI_GREENMISSILE_MUL_X,
                                  (f32) fp->mv.lg.SpecialS.chargeFrames,
