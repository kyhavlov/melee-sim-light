...
void ftYs_Init_OnLoad(HSD_GObj* gobj)
{
    ...
    // The costume visibility models feeding the egg-shield matanim scan are
    // headless-absent by the ftParts_8007487C boundary (vis->xC[] stays
    // NULL). The scanned length still matters: retail publishes it through
    // ftAnim_8006F3DC's no-aobj fall-through in Yoshi's trackless motion
    // rows, and ftYs_Shield_8012C850 re-enters the guard at that published
    // frame. Derive the same value from the immutable costume matanim
    // archive data (all egg matanims share one end frame under the source's
    // "yoshi matanim frame not same" assert); the store must stay idempotent
    // because Match construction repeats OnLoad against the read-only DAT
    // page.
    // refs/melee/src/melee/ft/chara/ftYoshi/ftYs_Init.c::ftYs_Init_OnLoad
    (void) temp;
    (void) temp_r27;
    (void) temp_r28;
    {
        float matanim_end = msl_costume_matanim_end_frame(fp);
        if (other_attr->xC != matanim_end) {
            other_attr->xC = matanim_end;
        }
    }
#else
    if (!temp) {
