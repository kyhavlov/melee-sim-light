                                                       ...
                                                       bool facing_right)
{
    ...
    // GALE01 0x80131948-family (both walls of all four Coll handlers)
    // fuses the bump-height offset (fmadds f0, f1, f2, f0).
    pos.y = __fmadds(
        0.5F, ABS(fp->coll_data.ecb.top.y + fp->coll_data.ecb.bottom.y),
        pos.y);
...
{
    {
        ...
        // GALE01 0x8012FD5C/0x8012FD78/0x8012FD94/0x8012FDB0 fuse the
        // lean-angle affine in all four branches (fmadds f0, f4, f1, f0).
        if (x10 > 0.0F) {
            if (perm_vel_inline(fp) > 0.0F) {
                fp->mv.ys.specials.x18 =
                    __fmadds(abs_xE4, abs_vel / abs_x10, -xE4);
            } else {
                f32 scaled_abs_x10 = abs_x10;
                scaled_abs_x10 *= 0.7F;
                fp->mv.ys.specials.x18 =
                    __fmadds(abs_xE4, abs_vel / scaled_abs_x10, -xE4);
            }
        } else {
            if (perm_vel_inline(fp) < 0.0F) {
                fp->mv.ys.specials.x18 =
...

    {
        ...
        // GALE01 0x8012FE08 fuses the roll-period scale
        // (fmadds f2, f1, f2, f3).
        f32 total = __fmadds(0.7F, abs_x10, abs_x10_2);
        (void) total;
...
void ftYs_SpecialAirSLoop_0_Phys(Fighter_GObj* gobj)
{
    ...
    if (vel > 0.0F) {
        if (floor_nx > 0.0F) {
            ...
            // GALE01 0x80131158/0x8013115C fuse the uphill/downhill slope
            // factors (fmadds f5, f2, f1, f3; fnmsubs f6, f2, f4, f3).
            f5 = __fmadds(slope, dir * attributes->x94, 1.0F);
            f6 = __fnmsubs(slope, dir, 1.0F);
        } else {
        ...
        if (floor_nx < 0.0F) {
            f32 neg_dir = -dir;
            f32 slope = 1.0F - fp->coll_data.floor.normal.y;
            // GALE01 0x801311A0/0x801311A4 fuse the mirrored slope factors
            // (fmadds f5, f2, f1, f3; fnmsubs f6, f2, f4, f3).
            f5 = __fmadds(slope, neg_dir * attributes->x94, 1.0F);
            f6 = __fnmsubs(slope, neg_dir, 1.0F);
        } else {
            f32 temp = (1.0F - fp->coll_data.floor.normal.y) * -dir;
            f5 = 1.0F + temp;
            f6 = 1.0F - temp;
    ...
    // GALE01 0x80131258 fuses the terrain acceleration onto gr_vel
    // (fmadds f0, f30, f1, f0).
    fp->gr_vel =
        __fmadds(delta, mpLib_800569EC(fp->coll_data.floor.flags), fp->gr_vel);
...
void ftYs_SpecialAirSLoop_1_Phys(Fighter_GObj* gobj)
{
    Fighter* fp = GET_FIGHTER(gobj);
    PAD_STACK(8);
    // GALE01 0x8013131C fuses the stored roll speed onto gr_vel
    // (fmadds f0, f2, f1, f0).
    fp->gr_vel = __fmadds(fp->mv.ys.specials.x20,
