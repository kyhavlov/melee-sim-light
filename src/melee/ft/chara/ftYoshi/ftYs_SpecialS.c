...
#include <melee/ft/chara/ftCommon/inlines.h>
                                                       ...
                                                       bool facing_right)
{
    ...
    pos.y = __fmadds(0.5F,
                     ABS(fp->coll_data.ecb.top.y + fp->coll_data.ecb.bottom.y),
                     pos.y);
...
static inline void ftYs_SpecialS_UpdateScale2(Fighter_GObj* gobj)
{
    ...
    if (fp->mv.ys.specials.x8 >= 0 && fp->mv.ys.specials.x8 < 4) {
        scale.x = fp->fv.ys.x222C.x;
        scale.y = fp->fv.ys.x222C.y * ftYs_Unk3_803CED84[fp->mv.ys.specials.x8];
        scale.z = fp->fv.ys.x222C.z * ftYs_Unk3_803CED94[fp->mv.ys.specials.x8];
        HSD_JObjSetScale(jobj, &scale);
        fp->mv.ys.specials.x8++;
    } else {
        HSD_JObjSetScale(jobj, &fp->fv.ys.x222C);
    ...
    if (fp->mv.ys.specials.x10 > 0.0F) {
        if (perm_vel_inline(fp) > 0.0F) {
            fp->mv.ys.specials.x18 =
                __fmadds(abs_xE4, abs_vel / abs_x10, -attrs->xE4);
        } else {
            f32 scaled_abs_x10 = abs_x10;
            scaled_abs_x10 *= 0.7F;
            fp->mv.ys.specials.x18 =
                __fmadds(abs_xE4, abs_vel / scaled_abs_x10, -attrs->xE4);
        }
    } else {
        if (perm_vel_inline(fp) < 0.0F) {
            fp->mv.ys.specials.x18 =
...
void ftYs_SpecialAirSLoop_1_Anim(Fighter_GObj* gobj)
{
    ...
    {
        f32 total =
            __fmadds(0.7F, ABS(fp->mv.ys.specials.x10),
...
void ftYs_SpecialAirSLoop_0_Phys(Fighter_GObj* gobj)
{
    ...
    fp->gr_vel = __fmadds(delta, mpLib_800569EC(fp->coll_data.floor.flags),
                          fp->gr_vel);
