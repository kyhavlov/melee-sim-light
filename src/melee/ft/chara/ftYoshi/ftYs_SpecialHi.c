...
void ftYs_SpecialS_8012DF8C(Fighter_GObj* gobj, Vec3* arg1)
{
    ...
    }
    {
        ...
        // GALE01 0x8012E0A0/0x8012E0D8 fuse the charge-speed affine onto
        // both velocity lanes (fmadds f0, f3, f2, f0).
        arg1->x = __fmadds((f32) fp->mv.ys.specialhi.x4, da->x100, da->xFC) *
                  cosf(angle);
        arg1->y = __fmadds((f32) fp->mv.ys.specialhi.x4, da->x100, da->xFC) *
                  sinf(angle);
        arg1->z = 0.0f;
    }
}
...
void fn_8012E110(Fighter_GObj* gobj)
{
    ...
    if (fp->cmd_vars[0] != 0U && fp->fv.ys.x2238 != NULL) {
        ...
        ftYs_SpecialS_8012DF8C_outline(gobj, &sp18);
        {
            ...
            // GALE01 0x8012E23C fuses the charge-spin affine
            // (fmadds f1, f2, f1, f0).
            it_802B28C8(fp->fv.ys.x2238, &sp18, &sp24,
                        __fmadds(x4, da->x110, da->x10C), x4);
        }
        fp->fv.ys.x2238 = NULL;
        fp->take_dmg_cb = NULL;
        fp->death2_cb = NULL;
