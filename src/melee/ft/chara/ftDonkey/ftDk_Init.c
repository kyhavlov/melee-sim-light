...
void ftDk_Init_OnLoad(HSD_GObj* gobj)
{
    ...
    {
        float x8 = lbAnim_8001E8F8(ftData_80085E50(fp, 296));
        float xC = lbAnim_8001E8F8(ftData_80085E50(fp, 297));
        float x10 = lbAnim_8001E8F8(ftData_80085E50(fp, 298));

        // The source derives these three archive attributes during fighter
        // load. GameData preload performs the first mutation; later Match
        // construction repeats the derivation but must not write the
        // now-read-only DAT page. Preserve source evaluation/PUSH order and
        // skip only an idempotent store.
        // refs/melee/src/melee/ft/chara/ftDonkey/ftDk_Init.c::ftDk_Init_OnLoad
        if (ftData_attr->x8 != x8) {
            ftData_attr->x8 = x8;
        }
        if (ftData_attr->xC != xC) {
            ftData_attr->xC = xC;
        }
        if (ftData_attr->x10 != x10) {
            ftData_attr->x10 = x10;
        }
    }
#else
...
void ftDk_Init_8010DB3C(HSD_GObj* gobj)
{
    ...
    if (ftCheckThrowB3(fp) && (colldata->env_flags & Collide_FloorMask)) {
        ...
        int i;
        for (i = 0; i < 4; i++) {
            // GALE01 0x8010DC00 fuses the index product into the centering
            // subtract (fmsubs f1, f4, f1, f3).
            float temp_f5 = __fmsubs(donkey_attr->SpecialLw.x68, i,
