...
HSD_Joint* ftKb_SpecialN_800F5898(Fighter_GObj* gobj)
{
    ...
    // Hosted: retail reads x48_items + 0x10, the fifth 4-byte pointer; index
    // the list so the walk lands on the same entry on every pointer width.
...
{
    // Hosted: retail reads fp + 0x840, coll_data.floor.flags; the hosted
    // Fighter widens earlier members, so name the lane.
