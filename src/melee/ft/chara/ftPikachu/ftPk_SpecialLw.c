...
void ftPk_SpecialLw_Enter(HSD_GObj* gobj)
{
    ...
    // The source zeroes the thunder article pointer through its 32-bit
    // specialhi.x0 alias; the hosted full-width slot needs the actual
    // owner lane named. PPC behavior is unchanged.
...
void ftPk_SpecialAirLw_Enter(HSD_GObj* gobj)
{
    ...
    fp->throw_flags = 0;
    fp->mv.pk.specialhi.x4 = 1;
    // See ftPk_SpecialLw_Enter: the hosted thunder-pointer slot is zeroed
    // through its actual owner lane. PPC behavior is unchanged.
    fp->mv.pk.speciallw.x0 = NULL;
