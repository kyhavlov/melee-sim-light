...
void ftCo_8008DCE0(Fighter_GObj* gobj, int arg1, float facing_dir)
{
    ...
    block_23:
        ...
        msid = ((int (*)[4][3])
                    ftCo_803C5520)[0][kb_level][fp->dmg.x184c_damaged_hurtbox];
        if (!(temp_f1_2 >
              (float) (M_PI_2 + (double) p_ftCommonData->x1E8_radians)))
        {
            goto block_26;
        }
    ...
    block_28:
        ...
        goto block_36;
    block_33:
        if (!(fp->dmg.x1830_percent >= (float) p_ftCommonData->x23C))
        {
            goto block_36;
        }
...
block_44:
    ...
    // The current decomp call has these two arguments transposed. Retail
    // loads the damage element from fp+0x1860 and retains the original
    // knockback tier in r28 when selecting the color-animation entry.
    // refs/melee/build/GALE01/asm equivalent: data/raw/main.dol
    //     GALE01 0x8008E1A0..0x8008E228
...
block_62:
    var_r0 = 0;
block_63:
    // refs/melee/src/melee/ft/{types.h,chara/ftCommon/types.h}
    // Named owners preserve these PPC fields when native pointers widen.
    fp->mv.co.damage.x4 = var_r0;
...
block_70:
    ...
    if (!(temp_f30 >= p_ftCommonData->x20C)) {
        goto block_73;
    }
    fp->dmg.x1908 = 0x4F;
    ...
    goto block_75;
block_73:
    if (!(temp_f30 >= p_ftCommonData->x208)) {
        goto block_75;
    }
    fp->dmg.x1908 = 0x50;
...
block_75:
    ...
    if (!(temp_f30 >= p_ftCommonData->x174)) {
        goto block_79;
    }
    {
        enum_t cam_enum = 4;
...
void ftCo_8008E5A4(Fighter* fp)
{
    if (fp->input.lstick.x || fp->input.lstick.y) {
        ...
        // Retail GALE01 0x8008E5F4..0x8008E5FC keeps the squared
        // magnitude's final add fused.
        float kb_y_sq = kb_y * kb_y;
        float kb_mag = __fmadds(kb_vel_x_neg, kb_vel_x_neg, kb_y_sq);
        if (!(kb_mag < 0.00001f)) {
            // Retail GALE01 0x8008E618..0x8008E624.
            float f3 = __fmadds(kb_y, fp->input.lstick.x,
            ...
            }
            {
                ...
                float scale;
                // Retail GALE01 0x8008E660 computes the radicand with an
                // fmadd before the inlined MSL sqrt sequence.
                kb_mag = sqrtf(__fmadds(kb_x, kb_x, kb_y_sq));
                scale = deg_to_rad * p_ftCommonData->x1A8;
                // Retail GALE01 0x8008E6CC.
                angle = __fmadds(scale, f30, angle);
