...
void ftColl_80078538(Fighter_GObj* gobj, Vec3* pos, u32 sfx_severity,
                     float damage, float scale)
{
    ...
    if (scale < p_ftCommonData->x3F0) {
        efSync_Spawn(0x3E8, 0, pos, &damage);
    } else {
        efSync_Spawn(0x3F3, 0, pos);
    }

    // GALE01 0x800785A0 tests integer r5, while the effect scale passed in
    // f1 is stored at 0x14(r1). Callers at 0x8007A184/0x8007A200 and
    // 0x8007A434/0x8007A4B4 pass HitCapsule::sfx_severity in r5 and the
    // converted DmgLog damage in f1.
    if (sfx_severity >= 1) {
...
void ftColl_8007A06C(Fighter_GObj* gobj, void* dmg_ptr, void* log, size_t idx,
                     int arg4)
{
    // Both Fighter damage-result overlays begin on a PPC 4-byte boundary.
    // Keep their explicit source layout when native pointers widen rather
    // than adding pointer padding relative to an assumed 8-byte-aligned base.
    // refs/melee/src/melee/ft/{ftcoll.c,types.h}
    struct __attribute__((packed, aligned(4))) DmgResult {
        /* 0x00 */ float dir;
        /* 0x04 */ int angle;
    ...
    for (i = 0; i < idx; i++) {
        ...
        case 1: {
            ...
            if (arg4 != 0) {
                ...
                switch (sfx_id) {
                case 0x3E8:
                    ftColl_80078538(gobj, &entry->pos, hit->sfx_severity,
                                    entry->x20, kb);
                    break;
                case 0x3E9:
                case 0x3EA:
        ...
        case 2: {
            ...
            if (hit->x28 != 0) {
                ...
                decay -= result;
                damage = (float) count + fp->dmg.x1838_percentTemp;
                growth = (float) (u32) entry->size_of_xC * damage;
                growth = ftd->x114 * growth;
                growth = __fmadds(ftd->x110, damage, growth);
                result = decay * growth;
                result = __fmadds(ftd->x11C, result, ftd->x120);
        ...
        case 3: {
            ...
            if (stack_hit.x28 != 0) {
                ...
                growth = ftd->x118 * (float) (u32) stack_hit.x28;
                growth = ftd->x114 * growth;
                growth = __fmadds(ftd->x118, ftd->x110, growth);
                result = decay * growth;
                result = __fmadds(ftd->x11C, result, ftd->x120);
                x24_f = 0.01F * (float) (u32) stack_hit.x24;
                result = __fmadds(x24_f, result, (float) (u32) stack_hit.x2C);
                result = stage * result;
                result = attack * result;
                result = defense * result;
            } else {
                ...
                decay -= result;
                damage = (float) count + fp->dmg.x1838_percentTemp;
                growth = (float) (u32) stack_hit.unk_count * damage;
                growth = ftd->x114 * growth;
                growth = __fmadds(ftd->x110, damage, growth);
                result = decay * growth;
                result = __fmadds(ftd->x11C, result, ftd->x120);
...
void ftColl_8007B62C(Fighter_GObj* gobj, bool arg1)
{
    Fighter* fp = GET_FIGHTER(gobj);
    fp->x1988 = arg1;
    switch (arg1) {
