...
// ftLoadCommonData is one scalar block with no archive relocations. Preserve
// still-unknown slots as 32-bit source words rather than host-sized pointers.
// refs/melee/src/melee/ft/fighter.c::Fighter_FirstInitialize_80067A84
struct ftCommonData {
    /*   +0 */ float x0;
    /*   +4 */ float x4;
...
struct ftData {
    ...
    /* +20 */ struct {
        // Guard pose variants consumed as an HSD_Joint pointer array.
        // refs/melee/src/melee/ft/chara/ftCommon/ftCo_Guard.c::ftCo_80091E78
        /* +0 */ HSD_Joint** x0;
        /* +4 */ HSD_Joint* x8;
    }* x20;
    // refs/melee/src/melee/ft/{ftwaitanim.c,ft/types.h::WaitStruct}
    /* +24 */ WaitStruct* x24;
    /* +28 */ WaitStruct* x28;
    ...
    /* +4C */ FtSFX* x4C_sfx;
    /* +50 */ Vec2* x50;
    // Five-entry fighter-part cycle used by the 0x8D effect anchor.
    // refs/melee/src/melee/ft/chara/ftCommon/ftCo_09F7.c::ftCo_8009F834
    /* +54 */ int* x54;
...
// Motion-state tables initialize this union as one PPC source word. Decode
// its authored fields numerically so bitfield allocation order is host-neutral.
// refs/melee/src/melee/ft/{types.h,ftmotionstates.c}
static inline u8 MotionState_MoveId(const struct MotionState* state)
{
    return (u8) (state->_ >> 24);
}

static inline bool MotionState_Flag(const struct MotionState* state, u32 bit)
{
    return ((state->_ >> (23U - bit)) & 1U) != 0;
}
...
// The animation metadata stores its flags as one PPC source word.
// refs/melee/src/melee/ft/{ftdata.c,ftanim.c}
static inline bool Fighter_WaitAnimFlag(
    const struct Fighter_WaitAnimData* data, u32 bit)
{
    return (((u32) data->x10_animCurrFlags >> (31U - bit)) & 1U) != 0;
}
...
/* fp+2070 */ union Struct2070 {
    /* fp+2070 */ struct {
        ...
        /*
         * x2070_int is a numeric source word. Reverse its byte/bitfield
         * declaration on little-endian hosts so the named fields retain their
         * PPC meanings. refs/melee/src/melee/ft/types.h::Struct2070
         */
        /* fp+2073 */ u8 x2073;
...
struct Fighter {
    ...
    /* fp+182C */ struct dmg {
        ...
        /* fp+186C */ int x186c;
        // Second DmgResult overlay written by ftColl_8007A06C.  Its first
        // word is direction, not a pointer.
