...
{
    // Matching source compiles to fmadds at GALE01 0x8000DC74,
    // 0x8000DC8C, and 0x8000DC9C. This owner drives shield-bounced article
    // velocity, so preserve the retail scalar-single accumulation on host.
    // refs/melee/src/melee/lb/lbvector.c::lbVector_Mirror
    float dot = __fmadds(unit_mirror_axis->x, a->x,
                         unit_mirror_axis->y * a->y);
    float f = dot * -2.0f;

    a->x = __fmadds(unit_mirror_axis->x, f, a->x);
    a->y = __fmadds(unit_mirror_axis->y, f, a->y);
}
