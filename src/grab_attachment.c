...
enum { MSL_FTPART_RHANDN = 39 };  // FtPart_RHandN (proxy grab/throw anchor; SSANIM01 v3 stored)
...
void grab_attachment_update_post_collision(MslBatch* batch) {
  ...
  for (int bi = 0; bi < batch->batch_size; bi++) {
    for (int p = 0; p < num_players; p++) {
      ...
      // - ftCo_Thrown.c::ftCo_800DE508 uses x1A70.z as the "forward" offset term, applied onto pos.x
      //   with facing_dir, and adds x1A70.y to pos.y.
