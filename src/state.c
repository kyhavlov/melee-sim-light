...
int state_alloc(MslStateSoA* state, int batch_size) {
  ...
  state->guard_entry_source_action_id = (uint16_t*)alloc_aligned_64(sizeof(uint16_t) * bp);
  ...
  if (!state->frame_id || !state->frame_pre_random_seed || !state->stage_id ||
      ...
      !state->guard_entry_source_action_id || !state->guard_reflect_timer_x14 ||
...
void state_free(MslStateSoA* state) {
  ...
  alloc_free(state->damageflyroll_fighter_8006cda4_phase_hint);
  ...
  alloc_free(state->guard_entry_source_action_id);
