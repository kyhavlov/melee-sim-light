...
#undef MSL_STATE_FIELD
  ...
  memset(state->collision_pose_valid, 0, sizeof(uint8_t) * bp);
  memset(state->collision_pose_facing, 0, sizeof(uint8_t) * bp);
  memset(state->collision_pose_action_id, 0, sizeof(uint16_t) * bp);
  memset(state->collision_pose_msid, 0, sizeof(uint16_t) * bp);
  memset(state->collision_pose_anim_frame, 0, sizeof(float) * bp);
  ...
  memset(state->grab_constraint_x2174_x, 0, sizeof(float) * bp);
  memset(state->grab_constraint_x2174_y, 0, sizeof(float) * bp);
  memset(state->grab_constraint_x2174_z, 0, sizeof(float) * bp);
  memset(state->grab_constraint_x2174_valid, 0, sizeof(uint8_t) * bp);
  ...
  memset(state->script_msid, 0xFF, sizeof(uint16_t) * bp);
  memset(state->script_event_cursor, 0, sizeof(uint32_t) * bp);
  memset(state->script_last_frame_fp_q16_16, 0, sizeof(int32_t) * bp);
  memset(state->script_cmd_vars, 0, sizeof(uint16_t) * bp4);
  memset(state->script_throw_flags, 0, sizeof(uint8_t) * bp);
  memset(state->script_hit_status_x1988, 0, sizeof(uint8_t) * bp);
  memset(state->script_hurtcap_state, 0, sizeof(uint8_t) * bpc);
  memset(state->script_state_flags_221c_y, 0, sizeof(uint16_t) * bp);
  memset(state->script_bone_physics_mask, 0, sizeof(uint64_t) * bp);
  memset(state->script_throw_hitbox_valid, 0, sizeof(uint8_t) * bp * 2u);
  memset(state->script_throw_hitbox_damage, 0, sizeof(float) * bp * 2u);
  memset(state->script_throw_hitbox_angle, 0, sizeof(uint16_t) * bp * 2u);
  memset(state->script_throw_hitbox_kbg, 0, sizeof(uint16_t) * bp * 2u);
  memset(state->script_throw_hitbox_wsk, 0, sizeof(uint16_t) * bp * 2u);
  memset(state->script_throw_hitbox_bkb, 0, sizeof(uint16_t) * bp * 2u);
  memset(state->script_throw_hitbox_element, 0, sizeof(uint8_t) * bp * 2u);
  memset(state->script_throw_hitbox_sfx_kind, 0, sizeof(uint8_t) * bp * 2u);
  memset(state->script_throw_hitbox_sfx_severity, 0, sizeof(uint8_t) * bp * 2u);
  ...
  for (size_t i = 0; i < bph; i++) {
    ...
    state->hitbox_capsule_state[i] = 0u;
    state->hitbox_only_hit_grabbed[i] = 0u;
    state->hitbox_offset_x[i] = 0.0f;
    state->hitbox_offset_y[i] = 0.0f;
    state->hitbox_offset_z[i] = 0.0f;
