...
int state_alloc(MslStateSoA* state, int batch_size) {
  ...
  state->stage_yoshi_shyguy_spawn_rng_seed = (uint32_t*)alloc_aligned_64(sizeof(uint32_t) * b);
  ...
  state->coll_ecb_top_rel_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_ecb_left_rel_x = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_ecb_right_rel_x = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_ecb_side_rel_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_prev_ecb_bottom_rel_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_prev_ecb_top_rel_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_prev_ecb_left_rel_x = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_prev_ecb_right_rel_x = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_prev_ecb_side_rel_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_desired_ecb_bottom_rel_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_desired_ecb_top_rel_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_desired_ecb_left_rel_x = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_desired_ecb_right_rel_x = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_desired_ecb_side_rel_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  ...
  state->coll_floor_result_normal_x = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_floor_result_normal_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_substep_prev_pos_x = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_substep_prev_pos_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_substep_cur_pos_x = (float*)alloc_aligned_64(sizeof(float) * bp);
  state->coll_substep_cur_pos_y = (float*)alloc_aligned_64(sizeof(float) * bp);
  ...
  state->match_flow_pending_rebirth_char_id = (uint8_t*)alloc_aligned_64(sizeof(uint8_t) * bp);
  ...
  state->cliff_ledge_floor_segment_id = (uint16_t*)alloc_aligned_64(sizeof(uint16_t) * bp);
  state->cliff_ledge_floor_segment_seeded = (uint8_t*)alloc_aligned_64(sizeof(uint8_t) * bp);
  ...
  state->x672_input_timer_frame_start = (uint8_t*)alloc_aligned_64(sizeof(uint8_t) * bp);
  ...
  state->frame_start_instance_id = (uint16_t*)alloc_aligned_64(sizeof(uint16_t) * bp);
  ...
  if (!state->frame_id || !state->frame_pre_random_seed || !state->stage_id ||
      ...
      !state->coll_ecb_top_rel_y || !state->coll_ecb_left_rel_x || !state->coll_ecb_right_rel_x ||
      !state->coll_ecb_side_rel_y || !state->coll_prev_ecb_bottom_rel_y ||
      !state->coll_prev_ecb_top_rel_y || !state->coll_prev_ecb_left_rel_x ||
      !state->coll_prev_ecb_right_rel_x || !state->coll_prev_ecb_side_rel_y ||
      !state->coll_desired_ecb_bottom_rel_y || !state->coll_desired_ecb_top_rel_y ||
      ...
      !state->match_flow_timer || !state->match_flow_pending_rebirth_char_id ||
      !state->dead_up_fall_offset_x || !state->dead_up_fall_offset_y ||
      ...
      !state->frame_start_instance_id || !state->attack_identity_last_action_id ||
...
void state_free(MslStateSoA* state) {
  ...
  alloc_free(state->stage_yoshi_shyguy_spawn_rng_seed);
  ...
  alloc_free(state->coll_ecb_top_rel_y);
  alloc_free(state->coll_ecb_left_rel_x);
  alloc_free(state->coll_ecb_right_rel_x);
  alloc_free(state->coll_ecb_side_rel_y);
  alloc_free(state->coll_prev_ecb_bottom_rel_y);
  alloc_free(state->coll_prev_ecb_top_rel_y);
  alloc_free(state->coll_prev_ecb_left_rel_x);
  alloc_free(state->coll_prev_ecb_right_rel_x);
  alloc_free(state->coll_prev_ecb_side_rel_y);
  alloc_free(state->coll_desired_ecb_bottom_rel_y);
  alloc_free(state->coll_desired_ecb_top_rel_y);
  alloc_free(state->coll_desired_ecb_left_rel_x);
  alloc_free(state->coll_desired_ecb_right_rel_x);
  alloc_free(state->coll_desired_ecb_side_rel_y);
  ...
  alloc_free(state->coll_floor_result_normal_x);
  alloc_free(state->coll_floor_result_normal_y);
  alloc_free(state->coll_substep_prev_pos_x);
  alloc_free(state->coll_substep_prev_pos_y);
  alloc_free(state->coll_substep_cur_pos_x);
  alloc_free(state->coll_substep_cur_pos_y);
  ...
  alloc_free(state->match_flow_pending_rebirth_char_id);
  ...
  alloc_free(state->cliff_ledge_floor_segment_id);
  alloc_free(state->cliff_ledge_floor_segment_seeded);
  ...
  alloc_free(state->x672_input_timer_frame_start);
  ...
  alloc_free(state->frame_start_instance_id);
