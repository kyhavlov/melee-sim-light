...
static void write_item_visuals(const MslCoreMatch* match, uint8_t* out)
{
    ...
    while (gobj != NULL && slot < MSL_CORE_MAX_ITEMS) {
        ...
        uint8_t* visual = out + (size_t) slot * sizeof(MslViewerItemVisual);

        if (ip != NULL &&
            (ip->kind == It_Kind_Mewtwo_ShadowBall ||
             ip->kind == It_Kind_Samus_Charge ||
             ip->kind == It_Kind_Kirby_MewtwoShadowBall ||
             ip->kind == It_Kind_Kirby_SamusCharge) &&
            joint != NULL)
        {
        ...
        if (joint != NULL) {
            ...
            if (matrix != NULL) {
                put_f32(visual, offsetof(MslViewerItemVisual, x),
                        matrix[0][3]);
                put_f32(visual, offsetof(MslViewerItemVisual, y),
                        matrix[1][3]);
                put_f32(visual, offsetof(MslViewerItemVisual, z),
                        matrix[2][3]);
                put_f32(visual, offsetof(MslViewerItemVisual, scale),
                        joint_uniform_scale(joint));
                visual[offsetof(MslViewerItemVisual, valid)] = 1;
            }
        }
        if (ip != NULL) {
            ...
            if (tip != NULL) {
                ...
                if (tip->pos.x != 0.0F || tip->pos.y != 0.0F ||
                    tip->pos.z != 0.0F)
                {
                    put_f32(visual, offsetof(MslViewerItemVisual, tip_x),
                            tip->pos.x);
                    put_f32(visual, offsetof(MslViewerItemVisual, tip_y),
                            tip->pos.y);
                    visual[offsetof(MslViewerItemVisual, tip_valid)] = 1;
                }
            }
        }
        if (ip != NULL) {
            ...
            for (h = 0; h < 4; ++h) {
                ...
                put_f32(visual, offsetof(MslViewerItemVisual, hitbox_x),
                        hit->x4C.x);
                put_f32(visual, offsetof(MslViewerItemVisual, hitbox_y),
                        hit->x4C.y);
                put_f32(visual,
                        offsetof(MslViewerItemVisual, hitbox_radius),
                        hit->scale);
                break;
            }
        }
