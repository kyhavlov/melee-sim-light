...
// Fixed versus-rule projection for the Phase 2 two-human Fox/FD domain. These
...
void ftKb_SpecialN_800F1F1C(Fighter_GObj* gobj, Vec3* pos)
{
    ...
    // only when kind == FTKIND_KIRBY, so the Fox Phase 2 projection is an
...
// refs/melee/src/melee/gm/gm_16AE.c::gm_8016B094. The Phase 2 match is the
...
void gm_80167320(int slot, bool subchar)
{
    ...
    if (Player_GetFlagsBit1(slot) != 0) {
        // Spectator/wireframe teardown is outside the two-human Phase 2
...
bool msl_phase2_match_is_over(void) { return msl_match_ended; }
...
// guarded by FTKIND_KIRBY. The declared Phase 2 domain contains Fox only.
...
// Source stage accessors. Phase 2 initializes stage_info directly from the
