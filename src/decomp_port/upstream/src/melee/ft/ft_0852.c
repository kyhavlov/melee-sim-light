...
void ft_800852B0(void)
{
    ...
    // The original walks adjacent linker-ordered .data blocks by fixed byte
    // offsets. Hosted ELF does not preserve that layout; all projected Phase
    // 1 registry storage is BSS-zeroed, so initialize the named owners only.
    int i;
    for (i = 0; i < FTKIND_MAX; ++i) {
        gFtDataList[i] = NULL;
    }
    for (i = 0; i < 6; ++i) {
        ft_800852B0_Reset_ft_8045993C(i);
    }
#else
