...
void ftCo_DeadUpFall_Anim(Fighter_GObj* gobj)
{
    ...
    if (fp->mv.co.unk_deadup.x40 == 0) {
        ...
        case 2:
            ...
            // Slippi netplay's FreezeDeadUpFallPhysics/InitHitVelocity.asm
            // moves the camera-KO Y/Z velocity into the otherwise-unused
            // DeadUpFall overlay and zeros ordinary physics velocity. This
            // removes render-camera state from rollback gameplay.
...
void ftCo_DeadUpFall_Phys(Fighter_GObj* gobj)
{
...
#ifdef MSL_DECOMP_PORT
    {
        ...
        // Direct C projection of Slippi netplay's
        // FreezeDeadUpFallPhysics/UpdateFallVelocity.asm. The custom vector
        // advances without publishing camera-KO motion through self_vel.
