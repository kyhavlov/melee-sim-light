...
int msl_ecb_world_points_sample_collision_pose_f32(MslEcbWorldPoints* out, const MslBatch* batch,
  ...
  for (uint16_t pi = 0; pi < ch->ecb_joint_count; pi++) {
    ...
    float m[12];
    if (anim_pose_get_collision_matrix_f32(batch, player_idx, msid, anim_frame, part_id, m) != 0) {
      return -1;
    }
    ...
    const float rel_x_unmirrored = z * model_scale;
    const float rel_y = y * model_scale;
