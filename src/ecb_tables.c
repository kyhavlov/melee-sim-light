...
static const uint32_t k_ecb_bottom_format_version = 4;
static const uint32_t k_ecb_extents_format_version = 5;

uint32_t ecb_bottom_tables_format_version(void) { return k_ecb_bottom_format_version; }

uint32_t ecb_extents_tables_format_version(void) { return k_ecb_extents_format_version; }
...
static int load_table_for_char_into(MslEcbTable* table_by_char, const char* data_dir,
                                    const char* rel_path, uint8_t char_id) {
  char path[512];
  ...
  if (ver != k_ecb_bottom_format_version) {
    alloc_free(buf);
    return -1;
  }
  ...
  if (table_by_char[char_id].buf) {
    alloc_free(table_by_char[char_id].entry_index_by_msid);
...
enum { ECB_EXTENTS_STRIDE_BYTES = 16 };  // float32[4]
...
static int load_extents_table_for_char(const char* data_dir, const char* rel_path,
  ...
  const uint32_t ver = read_u32_le(buf + 8);
  if (ver != k_ecb_extents_format_version) {
    alloc_free(buf);
    return -1;
  }
  ...
  if (stride != (uint16_t)ECB_EXTENTS_STRIDE_BYTES) {
    alloc_free(buf);
    return -1;
  }
  const uint32_t toc_off = read_u32_le(buf + 16);
...
static const MslEcbExtentsTable* extents_table_for_char(uint8_t char_id) {
  if (!g_extents_loaded) {
    return NULL;
  }
  const MslEcbExtentsTable* t = &g_extents_table_by_char[char_id];
  if (!t->have || t->buf == NULL || t->entries == NULL || t->anim_count == 0) {
    return NULL;
  }
  return t;
...
MslEcbExtentsRel msl_ecb_extents_rel(uint8_t char_id, uint32_t animation_index, int action_frame) {
  ...
  const MslEcbExtentsTable* t = extents_table_for_char(char_id);
  if (t == NULL) {
    return (MslEcbExtentsRel){0};
  }

  const uint16_t msid = (uint16_t)animation_index;
  const int ei = find_extents_entry_index(t, msid);
  if (ei < 0) {
    return (MslEcbExtentsRel){0};
  }
