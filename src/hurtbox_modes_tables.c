#include "hurtbox_modes_tables.h"

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#include "alloc.h"

enum {
  HURTMODES_MAGIC_LEN = 8,
  HURTMODES_HDR_BYTES = 20,     // magic[8] + ver[u32] + frame_count[u16] + capsule_count[u16] + entry_count[u32]
  HURTMODES_VERSION_V1 = 1,
  INDEX_REC_BYTES_V1 = 12,      // msid[u16] + reserved[u16] + payload_bytes[u32] + payload_off[u32]
  PAYLOAD_REC_BYTES_V1 = 8,     // u64 packed states per frame
  MAX_CAPSULES = 32,            // MSL_MAX_HURTCAPS
};

static const uint8_t k_magic[HURTMODES_MAGIC_LEN] = {'M', 'S', 'L', 'H', 'U', 'R', 'M', '1'};

// Character id mapping follows Slippi post-frame `character` (GALE01):
// - Fox   = 1
// - Falco = 22
enum { MSL_CHAR_FOX = 1, MSL_CHAR_FALCO = 22 };

typedef struct {
  uint32_t* masks;  // [entry_count * frame_count]
  uint32_t mask_count;

  uint8_t* have_msid;            // [65536]
  uint32_t* base_index_by_msid;  // [65536] (index into masks[])

  uint16_t frame_count;
  uint16_t capsule_count;  // lanes stored in file (<=32)

  uint8_t have;
} MslHurtboxModesTable;

static MslHurtboxModesTable g_table_by_char[256];
static int g_loaded = 0;

static uint16_t read_u16_le(const uint8_t* p) {
  uint16_t v = 0;
  memcpy(&v, p, sizeof(v));
  return v;
}

static uint32_t read_u32_le(const uint8_t* p) {
  uint32_t v = 0;
  memcpy(&v, p, sizeof(v));
  return v;
}

static uint64_t read_u64_le(const uint8_t* p) {
  uint64_t v = 0;
  memcpy(&v, p, sizeof(v));
  return v;
}

static uint32_t mask_low_bits_u32(uint16_t n) {
  if (n >= 32) {
    return 0xFFFFFFFFu;
  }
  return (n == 0) ? 0u : ((1u << n) - 1u);
}

static void free_table(MslHurtboxModesTable* t) {
  if (t == NULL) {
    return;
  }
  alloc_free(t->masks);
  alloc_free(t->have_msid);
  alloc_free(t->base_index_by_msid);
  *t = (MslHurtboxModesTable){0};
}

static int load_for_char(const char* data_dir, const char* rel_path, uint8_t char_id) {
  char path[512];
  const int n = snprintf(path, sizeof(path), "%s/%s", data_dir, rel_path);
  if (n <= 0 || (size_t)n >= sizeof(path)) {
    return -1;
  }

  FILE* f = fopen(path, "rb");
  if (f == NULL) {
    return -1;
  }
  if (fseek(f, 0, SEEK_END) != 0) {
    fclose(f);
    return -1;
  }
  const long sz_long = ftell(f);
  if (sz_long <= 0) {
    fclose(f);
    return -1;
  }
  if (fseek(f, 0, SEEK_SET) != 0) {
    fclose(f);
    return -1;
  }

  const size_t sz = (size_t)sz_long;
  uint8_t* buf = (uint8_t*)alloc_malloc(sz);
  if (buf == NULL) {
    fclose(f);
    return -1;
  }
  const size_t got = fread(buf, 1, sz, f);
  fclose(f);
  if (got != sz) {
    alloc_free(buf);
    return -1;
  }

  if (sz < HURTMODES_HDR_BYTES) {
    alloc_free(buf);
    return -1;
  }
  if (memcmp(buf, k_magic, HURTMODES_MAGIC_LEN) != 0) {
    alloc_free(buf);
    return -1;
  }
  const uint32_t ver = read_u32_le(buf + 8);
  if (ver != (uint32_t)HURTMODES_VERSION_V1) {
    alloc_free(buf);
    return -1;
  }
  const uint16_t frame_count = read_u16_le(buf + 12);
  const uint16_t capsule_count = read_u16_le(buf + 14);
  const uint32_t entry_count = read_u32_le(buf + 16);
  if (frame_count == 0 || entry_count == 0) {
    alloc_free(buf);
    return -1;
  }
  if (capsule_count > (uint16_t)MAX_CAPSULES) {
    alloc_free(buf);
    return -1;
  }

  const size_t index_bytes = (size_t)entry_count * (size_t)INDEX_REC_BYTES_V1;
  const size_t index_end = (size_t)HURTMODES_HDR_BYTES + index_bytes;
  if (index_end > sz) {
    alloc_free(buf);
    return -1;
  }

  const uint32_t want_payload_bytes = (uint32_t)frame_count * (uint32_t)PAYLOAD_REC_BYTES_V1;

  // Validate index entries.
  for (uint32_t i = 0; i < entry_count; i++) {
    const size_t off = (size_t)HURTMODES_HDR_BYTES + (size_t)i * (size_t)INDEX_REC_BYTES_V1;
    if (off + (size_t)INDEX_REC_BYTES_V1 > sz) {
      alloc_free(buf);
      return -1;
    }
    const uint16_t _msid = read_u16_le(buf + off + 0);
    (void)_msid;
    const uint16_t reserved = read_u16_le(buf + off + 2);
    const uint32_t payload_bytes = read_u32_le(buf + off + 4);
    const uint32_t payload_off = read_u32_le(buf + off + 8);
    if (reserved != 0) {
      alloc_free(buf);
      return -1;
    }
    if (payload_bytes != want_payload_bytes) {
      alloc_free(buf);
      return -1;
    }
    if ((size_t)payload_off < index_end) {
      alloc_free(buf);
      return -1;
    }
    if ((size_t)payload_off + (size_t)payload_bytes > sz) {
      alloc_free(buf);
      return -1;
    }
  }

  uint8_t* have_msid = (uint8_t*)alloc_calloc(65536, 1);
  uint32_t* base_index_by_msid = (uint32_t*)alloc_calloc(65536, sizeof(uint32_t));
  uint32_t* masks = (uint32_t*)alloc_malloc((size_t)entry_count * (size_t)frame_count * sizeof(uint32_t));
  if (have_msid == NULL || base_index_by_msid == NULL || masks == NULL) {
    alloc_free(have_msid);
    alloc_free(base_index_by_msid);
    alloc_free(masks);
    alloc_free(buf);
    return -1;
  }

  // Decode payloads.
  for (uint32_t i = 0; i < entry_count; i++) {
    const size_t off = (size_t)HURTMODES_HDR_BYTES + (size_t)i * (size_t)INDEX_REC_BYTES_V1;
    const uint16_t msid = read_u16_le(buf + off + 0);
    const uint32_t payload_off = read_u32_le(buf + off + 8);

    if (have_msid[msid]) {
      alloc_free(have_msid);
      alloc_free(base_index_by_msid);
      alloc_free(masks);
      alloc_free(buf);
      return -1;
    }
    have_msid[msid] = 1;

    const uint32_t base = i * (uint32_t)frame_count;
    base_index_by_msid[msid] = base;
    if ((uint64_t)base + (uint64_t)frame_count > (uint64_t)entry_count * (uint64_t)frame_count) {
      alloc_free(have_msid);
      alloc_free(base_index_by_msid);
      alloc_free(masks);
      alloc_free(buf);
      return -1;
    }

    // Packed lane semantics: 2 bits per capsule, state 0 = HurtCapsule_Enabled.
    // Decomp: refs/melee/src/melee/lb/forward.h (HurtCapsuleState).
    for (uint16_t fi = 0; fi < frame_count; fi++) {
      const size_t roff = (size_t)payload_off + (size_t)fi * (size_t)PAYLOAD_REC_BYTES_V1;
      const uint64_t st = read_u64_le(buf + roff);
      uint32_t mask = 0u;
      for (uint16_t ci = 0; ci < capsule_count; ci++) {
        const uint64_t lane = (st >> (2u * (uint64_t)ci)) & 0x3u;
        if (lane == 0u) {
          mask |= (1u << ci);
        }
      }
      masks[base + (uint32_t)fi] = mask;
    }
  }

  alloc_free(buf);

  if (g_table_by_char[char_id].have) {
    free_table(&g_table_by_char[char_id]);
  }

  g_table_by_char[char_id] = (MslHurtboxModesTable){
      .masks = masks,
      .mask_count = (uint32_t)entry_count * (uint32_t)frame_count,
      .have_msid = have_msid,
      .base_index_by_msid = base_index_by_msid,
      .frame_count = frame_count,
      .capsule_count = capsule_count,
      .have = 1,
  };
  return 0;
}

int hurtbox_modes_tables_init(void) {
  if (g_loaded) {
    return 0;
  }

  const char* data_dir = getenv("MSL_DATA_DIR");
  if (data_dir == NULL || data_dir[0] == '\0') {
    data_dir = "data";
  }

  if (load_for_char(data_dir, "hurtbox_states/fox.bin", MSL_CHAR_FOX) != 0) {
    return -1;
  }
  if (load_for_char(data_dir, "hurtbox_states/falco.bin", MSL_CHAR_FALCO) != 0) {
    free_table(&g_table_by_char[MSL_CHAR_FOX]);
    return -1;
  }

  g_loaded = 1;
  return 0;
}

int hurtbox_modes_can_hit_mask(uint8_t char_id, uint16_t msid, uint16_t frame, uint16_t cap_count,
                               uint32_t* out_mask) {
  if (out_mask == NULL) {
    return -1;
  }
  *out_mask = 0u;
  if (!g_loaded) {
    return -1;
  }

  const MslHurtboxModesTable* t = &g_table_by_char[char_id];
  const uint32_t cap_mask = mask_low_bits_u32(cap_count);

  if (!t->have || t->masks == NULL || t->have_msid == NULL || t->base_index_by_msid == NULL ||
      t->frame_count == 0) {
    *out_mask = cap_mask;
    return 0;
  }

  if (!t->have_msid[msid]) {
    *out_mask = cap_mask;
    return 0;
  }

  uint16_t f = frame;
  if (f >= t->frame_count) {
    f = (uint16_t)(t->frame_count - 1);
  }

  const uint32_t base = t->base_index_by_msid[msid];
  if ((uint64_t)base + (uint64_t)f >= (uint64_t)t->mask_count) {
    *out_mask = cap_mask;
    return 0;
  }
  uint32_t mask = t->masks[base + (uint32_t)f];

  // If the file lane count is smaller than the runtime capsule count, treat missing lanes as
  // default-enabled (suite-neutral fallback).
  if (cap_count > t->capsule_count) {
    const uint32_t file_bits = mask_low_bits_u32(t->capsule_count);
    const uint32_t missing = cap_mask & ~file_bits;
    mask |= missing;
  }

  *out_mask = mask & cap_mask;
  return 0;
}

