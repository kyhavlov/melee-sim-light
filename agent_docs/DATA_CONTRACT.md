...
Policy:
- All gameplay-critical numbers come from **game files** (`SSBM.iso` /
  `$MSL_DATA_DIR/raw/*.dat`) and/or decomp.
- Extracted outputs live under `MSL_DATA_DIR` and are **generated artifacts**. ISO-derived files
  are gitignored; the source-authored Slippi neutral-spawn table is the sole tracked exception.
- The C core should load these at initialization (no parsing during `step`).

## Runtime Data Contract: Common Fighter Constants

`data/common/ft_common_data.json` is the generated, ISO-derived PlCo common-data artifact used for
small runtime-required constants. It is generated by
`tools/extraction/extract_ftcommon_data.py` from `$MSL_DATA_DIR/raw/PlCo.dat`; runtime loads it once during
...
- An authoritative per-HitCapsule lane may be non-empty or empty. Empty means the real
  ...
  group fallback.
- `combat_hitlist_hb_v2_mask[attacker][hitbox_id]` preserves fighter membership in the separate
  phantom/tip-log `HitCapsule.victims_2` ring. Bit `victim` materializes that live fighter in the
  exact capsule; CREATE copy, in-place mutation, and CLEAR follow the same source lifecycle as the
  capsule's `victims_1` ring.
...
- `refs/melee/src/melee/lb/lbcollision.c::{lbColl_8000ACFC,lbColl_80008688}` gate and insert
  `victims_1` entries.
- `refs/melee/src/melee/ft/ftcoll.c::{checkTipLog,inlineB0,inlineB1}` and
  `refs/melee/src/melee/lb/lbcollision.c::lbColl_80008820` gate and insert phantom/tip-log
  `victims_2` entries without suppressing a later full BODY contact.
...
## Replay Seed Contract: Destination-Frame HSD Stream

`MslSeed.frame_pre_random_seed` is the HSD stream at the start of the frame that `step_input`
executes. A validation row combines visible post-frame state from frame `i` with controller input
for frame `i+1`, so both `seed_t.frame_pre_random_seed` and `ref_t1.frame_pre_random_seed` contain
Slippi's frame-start seed for `i+1`.

This is a global scheduler contract. `SendFrameStart` creates a priority-zero player-class GObj and
records the seed before fighter animation and gameplay callbacks. Validation therefore installs
`rng[i+1]` for every row; Needle, DamageFlyRoll, Shy Guy, and other RNG consumers do not promote
selected rows or infer phase from their eventual outcome. Runtime source callbacks consume the one
live stream in their actual order. Free-running simulation starts from the configured seed and owns
subsequent advancement internally, while replay rollout reapplies each destination frame's recorded
seed before the corresponding step.

The separate `stage_yoshi_shyguy_spawn_rng_seed_u32/valid` lane remains for a rollout that begins
before a later zero-timer Shy Guy callback: it carries that future callback frame's authoritative
stream through the intervening simulated countdown. It is callback-local hidden source state, not
an exception to the per-transition frame-start contract.
...
- `refs/melee/src/melee/ft/fighter.c::{Fighter_Create_Inline2,Fighter_ProcessHit_8006D1EC}`.
- `refs/melee/src/melee/ft/chara/ftCommon/ftCo_Damage.c::ftCo_8008DCE0`.
...
- Held B and `motion_entry_instance_id_override_u16` are not sufficient latch proof. They can also
  ...
  derived B-edge action frame is inside the raw MSLFTSC1 cmd0 window. The live fighter-script
  cursor owns the separate runtime latch-clear tail.
...
- `shootNeedles` consumes `HSD_Randi(9)` from the ordinary destination-frame stream after
  `ftSk_SpecialNEnd_Anim` arms `mv.sk.specialn.x4`; Needle rows need no RNG-specific preprocessing.

Validation-buffer contract:
- These lanes expand `MslSeed`. Rerun validation from replay files after touching Needle
  count/timer lanes or stored Needle count persistence across non-SpecialN Sheik actions.
...
- `$MSL_DATA_DIR/raw/GrNLa.dat` (source)
...
Common constants:
- `$MSL_DATA_DIR/raw/PlCo.dat` (source)
...
- `$MSL_DATA_DIR/raw/PlFx.dat`, `$MSL_DATA_DIR/raw/PlFc.dat`,
  `$MSL_DATA_DIR/raw/PlCa.dat`, `$MSL_DATA_DIR/raw/PlMs.dat`,
  `$MSL_DATA_DIR/raw/PlSk.dat`, `$MSL_DATA_DIR/raw/PlZd.dat` (source)
- `data/characters/fox.json`, `data/characters/falco.json`, `data/characters/falcon.json`,
  ...
  - `static_x1a70_y` / `static_x1a70_z`: runtime-required `Fighter::x1A70` attachment vector.
    `tools/extraction/extract_character_attrs.py` composes the default-costume rest JObj tree and
    stores `FtPart_TransN.world - FtPart_XRotN.world`, matching the one-time fighter-create owner.
    These fields must not be reconstructed from Wait1: Marth, Falcon, and Sheik have source rest-Z
    components that their idle animation overwrites. Runtime applies the unscaled stored vector
    through `ftCo_800DE508`'s fighter-scale multiplication.
    Source: `refs/melee/src/melee/ft/fighter.c::Fighter_UnkUpdateVecFromBones_8006876C` and
...
- `data/attack_id/move_id/<char>.bin` (per-supported-character MotionState tables; ISO-derived,
  ...
  - Sources:
    - `$MSL_DATA_DIR/raw/main.dol` GALE01 `ftData_MotionStateList` at `0x803C2800`
    - `$MSL_DATA_DIR/raw/main.dol` GALE01 `ftData_CharacterStateTables` at `0x803C12E0`
...
- `data/stages/bin/{grnla,grnba,griz,grps,grst,grop}.bin` (stage collision/metadata; decomp-first, compact binary)
  ...
  - Sources:
    - `$MSL_DATA_DIR/raw/GrNLa.dat`, `GrNBa.dat`, `GrIz.dat`, `GrPs.dat`, `GrSt.dat`, and
      `GrOp.dat` public symbols `coll_data`, `grGroundParam`, and `map_head`
...
- `data/stage_items/yoshi_shyguy.bin` (Yoshi's Story Shy Guy stage-object item data; generated `MSLSTIO1` compact binary)
  - Purpose:
    ...
    - The separate seed lane `stage_yoshi_shyguy_spawn_rng_seed_u32/valid` carries the source
      ...
      `grStory_801E3418`.
  - Sources:
    - `$MSL_DATA_DIR/raw/GrSt.dat::yakumono_param` (`timer_min`, `timer_rand`, `spawnmany_rarity`, `vpos`)
    - `$MSL_DATA_DIR/raw/GrSt.dat::itemdata` Heiho `Article` common attrs / special attrs
      (`ItemAttr.x40` fixed ECB source, `ItemAttr.x60` scale)
    - `$MSL_DATA_DIR/raw/GrSt.dat` Heiho state-0 child `HSD_A_J_TRAY` FObjDesc interpreted through the
      extractor FObj port
  ...
  - Stale/non-v4 tables must be rejected by tooling readers; regenerate with
    `uv run python -m tools.extraction.extract_stage_item_objects --grst data/raw/GrSt.dat --out data/stage_items/yoshi_shyguy.bin --audit data/stage_items/yoshi_shyguy.json`
...
- `data/stage_items/dream_whispy.bin` (Dream Land Whispy wind data; generated `MSLWHSP1` compact binary)
  ...
  - Stale/non-v1 tables must be rejected by tooling readers; regenerate with
    `uv run python -m tools.extraction.extract_stage_item_objects --grop data/raw/GrOp.dat --dream-out data/stage_items/dream_whispy.bin --dream-audit data/stage_items/dream_whispy.json`
    or through `tools.extraction.build_data`.
- `data/scripts/<char>.bin` (per-supported-character unified decoded fighter script timeline;
  ...
  - Purpose:
    ...
    - The persistent fighter command interpreter consumes this binary artifact at runtime; the
      debug JSON remains extractor input/audit data, not a second gameplay owner.
  ...
  - Binary layout: `MSLFTSC1` v7 (v7 preserves every source control-timer command, including
    groups that contain no gameplay event)
    - `u8 magic[8] = "MSLFTSC1"`
    - `u32 version = 7`
    - `u32 entry_count`, `event_count`, `index_off`, `event_off`
    - index records: `msid`, reserved, first-event index, event count
    - event records: `u16 frame`, `u16 event_kind_id`, `u8 timer_kind`, reserved byte,
      `u16 timer_value`, `u32 payload_len`, then the kind-specific payload. `timer_kind` is zero for
      another command in the current group, one for source `SynchronousTimer` (`Command_01`), and
      two for source `AsynchronousTimer` (`Command_02`). Empty-payload events have length zero.
    - `data/scripts/<char>_manifest.json` maps event IDs to names and reports unknown event counts.
  - Generated/ignored; stale/non-v7 runtime tables must be regenerated. Python tooling readers may
...
- Source: each target's main/AJ/costume-rest DATs, `PlCo.dat` part tables, and Falcon donor
  `$MSL_DATA_DIR/raw/PlCa.dat` plus `PlCaAJ.dat` for `ftCo_SM_CaptureCaptain`.
...
projectile + hitbox params). SpecialN command-script shoot pulses are owned by MSLFTSC1 and
consumed by the live fighter-script cursor, not duplicated in this artifact.
...
- v6: removes SpecialN command-script shoot frames; runtime uses MSLFTSC1 `set_cmd_var(idx=2)`
  events through the live fighter-script cursor. Record size is 218 bytes.
...
The public command writes one ignored `MSL_DATA_DIR` root: original GALE01 archives and their hash
manifest under `raw/`, plus generated runtime tables elsewhere under the root. It requires the
complete character/stage registry; a partial manifest is not a native-runtime contract.

For derived-generator development, rebuild from an already extracted root with:

```bash
DATA_ROOT=${MSL_DATA_DIR:-data}
uv run python -m tools.extraction.build_data \
  --iso-dir "$DATA_ROOT/raw" \
  --raw-manifest "$DATA_ROOT/raw/manifest.json" \
  --out-dir "$DATA_ROOT"
```
