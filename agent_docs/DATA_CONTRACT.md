...
The same artifact carries all nine words of `p_ftCommonData+0x380`, the hit descriptor consumed by
`ftCo_800DE2F0`: state, damage, angle, KBG, WSK, BKB, element, and SFX severity/kind. These are
required loader keys. Capture release uses the extracted zero-damage/nonzero-knockback descriptor;
gameplay must not duplicate its BKB as a local constant. Source:
`refs/melee/src/melee/ft/chara/ftCommon/ftCo_Throw.c::ftCo_800DE2F0` and
...
- Generated by `tools/slippi/validation_buffer_builder.py::_derive_attackdash_x0_seed_lane`.
...
Source/generation:
- Generated by `tools/slippi/validation_buffer_builder.py` from same-frame GuardSetOff + both-fighter
...
Source/generation:
- Generated by `bindings/msl_preprocess_items.c::msl_derive_item_hidden_callback_seed_lanes_py`
  via `tools/slippi/validation_buffer_builder.py` when a same-owner same-identity laser in guard
...
Source/generation:
- Generated by `tools/slippi/validation_buffer_builder.py` from the selected `src_ports` list.
...
Source/generation:
- Generated by `tools/slippi/validation_buffer_builder.py` from game-start player static character data
...
Source/generation:
- Generated by `tools/slippi/validation_buffer_builder.py::_derive_phantom_damage_pending_seed_lanes`
...
Source/generation:
- Generated by `tools/slippi/validation_buffer_builder.py::_derive_jab_rapid_count_seed_lane` from
  replay action/input history.
- Runtime consumes the lane in `src/locomotion.c::grounded_attack_try_jab_chain_subset`.
- The per-character threshold is extracted as `data/characters/<char>.json::rapid_jab_window`.
- Attack100Loop continuation uses the extracted `set_throw_flags(hit_idx=0)` script checkpoints
  from `data/moves/<char>.json::ftCo_SM_Attack100Loop`.
- `attack100_x0[player]` and `attack100_x4[player]` reconstruct the two hidden
  `mv.co.attack100` latches for teacher-forced rows that begin inside Attack100Loop. Native
  preprocessing derives them prefix-causally from action-frame wraps, extracted script
  checkpoints, hitlag, and physical A press/release edges. Free-running runtime owns both latches
  directly through the Anim/IASA callbacks.
- A prior post-frame hitlag value of `1` exits at priority 0 before IASA; a later collision may
  re-enter hitlag in the same frame, so adjacent `1 -> N` post-frame snapshots still consume the
  current physical A edge.
...
## Replay Seed Contract: Grounded Falcon Kick On-Hit State

The replay seed contains `falcon_speciallw_hits[player]` and
`falcon_speciallw_friction[player]`, corresponding to grounded Falcon Kick's hidden
`mv.ca.speciallw.{x0,friction}` state.

- Grounded `ftCa_SpecialLw_Enter` initializes the count to zero and friction to `1.0`.
- `Fighter_ProcessHit` invokes Falcon's `deal_dmg_cb` once per frame when an accepted kick contact
  leaves `dmg.x1914` pending, including vulnerable/invincible BODY and item-hurtbox producers. The
  callback increments the bounded hit count and multiplies the cumulative friction by
  `data/characters/falcon.json::falcon_speciallw_on_hit_spd_modifier`.
- Native validation preprocessing reconstructs the action-instance-local prefix from the
  ProcessHit-owned `dmg.x1914` producers: attributed vulnerable BODY damage, invincible BODY
  contact, and fixed-history item damage. The no-percent producers use native, source-gated
  HitCapsule/hurtbox geometry and extracted item hurtboxes; hitlag onset or an unrelated item damage
  transition is not attribution. Fighter BODY collision remains eligible during an existing hitlag
  tail, with the per-HitCapsule victim list suppressing repeats. A simultaneous higher-priority
  incoming damage/phantom branch suppresses the callback, matching ProcessHit's branch order. A
  prefix-proven magnify counter wrap does not: magnify percent is applied after ProcessHit.
- Aerial Falcon Kick never installs this callback and does not own these lanes.

Decomp contract:
- `refs/melee/src/melee/ft/chara/ftCaptain/ftCa_SpecialLw.c::{ftCa_SpecialLw_Enter,ftCa_SpecialHi_800E400C,ftCa_Special_Inline_Friction}`.
- `refs/melee/src/melee/ft/fighter.c::Fighter_ProcessHit_8006D1EC`.
...
`ecb_lock_bottom_owner_u8[player]`, the hidden `CollData.desired_ecb.bottom.y` value and source owner
carried while `CollData_X130_Locked` is live.

Domain:
- `owner=1`: a valid generic replay-seeded lock, including `JumpAerialF/B -> EscapeAir` and a
  grounded Falcon special interrupted into Damage* after its collision callback. A reconstructed
  ten-frame timer alone is not proof of the Falcon special floor-loss writer.
- `owner=4`: prefix provenance proves a Falcon special callback called `ftCommon_8007D5D4` or
  `ftCommon_8007D60C`. This owner persists through the complete lock episode, including sustained
  Falcon-special and `Fall`/`FallSpecial` rows, until refresh, landing/unlock, or timer expiry.
- `owner=0`: the hidden bottom/owner is unavailable; reseed falls back to the current pose bottom.
- For either nonzero owner, the float is the prefix-causal desired ECB bottom reconstructed from
  extracted ECB tables and replay grounding / lock history.
...
Source/generation:
- Generated natively by `msl_binding.derive_ecb_lock_state` and
  `msl_binding.derive_ecb_lock_bottom_rel_y`, called from
  `tools/slippi/validation_buffer_builder.py` immediately after the lock prefix is available.
...
Source/generation:
- Generated by `tools/slippi/validation_buffer_builder.py::_derive_specialhi_rotate_model_seed_lane`.
...
Source/generation:
- Generated by `tools/slippi/validation_buffer_builder.py::_derive_fighter_8006cda4_pre_gate_consume_count_seed_lane`.
...
Seed generation:
- `tools/slippi/validation_buffer_builder.py` derives the global next counter from replay-visible
...
Derivation:
- Generated by `tools/slippi/validation_buffer_builder.py::_derive_sheik_vanish_travel_timer` from
...
Derivation:
- Generated by `tools/slippi/validation_buffer_builder.py::_derive_common_fall_blend_seed` from
...
Derivation:
- Generated by
  `tools/slippi/validation_buffer_builder.py::_derive_marth_counter_hitlag_floor_active` from
...
## Replay Seed Contract: Puff Multi-Jump Turnaround Window Counter

The replay seed contains `puff_mjump_turn_timer_u8[player]`, the post-frame value of the
multi-jump turnaround window counter (`mv.co.jumpaerial.x0`; the multi-jump arming site stores
through the shared mv slot) for the JumpAerialF1..F5 ladder (actions 341..345).
...
- `ftCo_800D74A4` arms `x2D0->x0` frames on ladder entry when the entry frame's processed stick x
  ...
  `ft_800CB6EC` tick.
- `ftCo_JumpAerialF1_Anim` runs `ft_800CB6EC` once per non-frozen frame; the counter decrements
  and the facing flip fires when it crosses `x2D0->x0 / 2`.

Derivation:
- Generated by `bindings/msl_validation_specials.c::msl_derive_puff_mjump_turn_timer_py`
  (wrapper `tools/slippi/seed_history.py::derive_puff_mjump_turn_timer`) from post action/char/
  hitlag/facing lanes plus the processed entry-frame stick; wired per slot in
  `tools/slippi/validation_buffer_builder.py`. Frames the fighter spent frozen under hitlag
  (previous row's post hitlag != 0, the `derive_run_x0` convention) do not tick.
- Attrs come from `data/characters/puff.json::{puff_mjump_turn_frames,puff_mjump_turn_threshold}`
  (`x2D0->x0/x4`).

Runtime consumption:
- `src/api.c` reseed copies the lane into `MslStateSoA.puff_mjump_turn_timer`; a teacher-forced
  row seeded inside the window keeps the pending mid-window facing flip.
- Free-running runtime arms and ticks the same counter through
  `msl_locomotion_puff_multijump_enter` and `puff_mjump_turn_tick`; the lane only initializes
  real hidden source state at reseed.

Validation-buffer contract:
- This lane expands `MslSeed`; rerun validation from replay files after touching it. Zero is the
  neutral value (no pending window) for non-ladder rows and pre-lane seeds.
...
## Replay Seed Contract: Puff Rollout Hidden Move Block

The replay seed contains the post-frame Rollout `mv.pr.specialn` block for the family 346..362:
- `puff_rollout_seed_valid_u8[player]` (0 = not reconstructable; reseed keeps its documented
  single-row approximations)
- `puff_rollout_charge_f32[player]` (`x2C`)
- `puff_rollout_turn_budget_i16[player]` (`x0`)
- `puff_rollout_angle_f32[player]` (`x14`, normalized [0, 2pi))
- `puff_rollout_pre_turn_vel_f32[player]` (`x10`)
- `puff_rollout_dir_i8[player]` (`x34.x`)
- `puff_rollout_facing_restore_i8[player]` (the mv facing_dir post-turn facing latch)

Owner (refs/melee/src/melee/ft/chara/ftPurin/ftPr_SpecialN.c):
- charge: xA0 at entry, += xA8 capped xA4 per Loop/Full Anim (the Loop->Full transition), -= xB4
  per Release Phys, *= xD4 floored 0 on the Release Coll wall bounce.
- budget: x34 at entry, -1 per Release/Turn Anim, -x38 on the deal-damage NHit handoff
  (`ftPr_SpecialS_8013D764`); Turn exhaustion flips the roll direction before the End handoff.
- angle: per-state deltas (Loop/Full charge-scaled xAC, Release 0.2 * x98, Turn 0.2 * x6C unroll;
  air variants scale by xBC); gates the natural end-of-roll pi crossing once the budget is spent.
- pre-turn velocity: gr_vel at the Release->Turn entry; the Turn exit test compares |gr_vel|
  against |x10 * xD0| after the sign crossing and consumes the facing latch.

Derivation:
- `bindings/msl_validation_specials.c::msl_derive_puff_rollout_seed_lanes_py` (wrapper
  `tools/slippi/seed_history.py::derive_puff_rollout_seed_lanes`), wired per slot in
  `tools/slippi/validation_buffer_builder.py`. The walk uses the ref action sequence as ground
  truth and applies each frame's lane effects from the state the frame STARTED in (transition
  rows carry the old state's effects plus entry-side extras). Hitlag-frozen frames (previous
  row's post hitlag != 0) hold every lane. Segments that begin mid-family (replay starts inside
  a roll) publish valid=0.
- Attrs come from `data/characters/puff.json::puff_rollout_*` (the ftPurinAttributes block).

Runtime consumption:
- `puff_specials_reseed_init` consumes the block when valid (slope lane x1C recomputed as
  -0.05 * x10 for grounded Turn rows, ground/air attr init elsewhere); invalid rows keep the
  documented velocity-inversion approximations.
- Free-running runtime owns the same lanes through `pr_rollout_update` and the Rollout Phys/Coll
  callbacks; the seed lanes only initialize hidden source state at reseed.

Related runtime-owner note (same packet): in-family Rollout transitions are frame-preserving
`Fighter_ChangeMotionState` calls and now run the motion-state identity bundle
(`msl_motion_state_enter_side_effects`); the Turn->Release exit additionally models the armed
`fp->x21EC = ftPr_SpecialS_8013D8B0` hook (`ft_80089824` unconditional instance bump +
`ft_800892A0` fresh attack instance).

Validation-buffer contract:
- These lanes expand `MslSeed`; rerun validation from replay files after touching them.

Decomp contract:
- `refs/melee/src/melee/ft/chara/ftPurin/ftPr_SpecialN.c` (the whole SpecialN family).
- `refs/melee/src/melee/ft/ft_0892.c::ft_800895E0` (x2073 byte gate; the family's shared x4 low
  byte keeps it quiet across in-family transitions).

## Replay Seed Contract: Jab-Combo Continuation Window

The replay seed contains `jab_input_window_f32[player]` (fp->hitlag_mul reused as the jab-combo
continuation counter) and `jab_unk_msid_u16[player]` (fp->unk_msid, the previous jab motion id),
post-frame.

Owner (refs/melee/src/melee/ft/chara/ftCommon/ftCo_Attack1.c):
- checkAttack11 arms co_attrs.jab_2_input_window and latches unk_msid = Attack11;
  doAttack12Normal re-arms jab_3_input_window (unk_msid = Attack12); doAttack13 leaves no
  further window.
- ftCo_Attack1_CheckInput: an A press from its neutral IASA callers while (window > 0 &&
  fp->x2218_b1) continues to Attack12/13 keyed on unk_msid; with no press the window
  decrements once per call.

Derivation: `bindings/msl_validation_specials.c::msl_derive_jab_combo_window_seed_lanes_py`
(builder-wired per slot with per-char attr LUTs from `data/characters/*.json::
{jab_2_input_window,jab_3_input_window}`, extracted from ftCo_DatAttrs +0x7C/+0x80).

Runtime consumption: reseed copies both lanes; the arm sites (jab entries) and the neutral
no-press decrement are live. The Wait-side CONTINUATION CONSUMER is currently PARKED after a
cross-suite regression (the modeled decrement call-set undercounts source's
ftCo_Attack1_CheckInput reach; see reports/triage/open_rollout_work_log.md item 6 for the
re-land plan). The consumer lock is a strict xfail until then.
...
Source/generation:
- `tools/slippi/validation_buffer_builder.py::_derive_landing_fallspecial_allow_interrupt_seed_lane`
...
Source/generation:
- `tools/slippi/validation_buffer_builder.py` derives the Turn seed lanes from replay-prefix action
...
Source/generation:
- Generated by `tools/slippi/validation_buffer_builder.py::_derive_grounded_overlap_hidden_pos_z`.
...
Characters (Fox/Falco/Falcon/Marth/Sheik/Zelda):
- `_iso/PlFx.dat`, `_iso/PlFc.dat`, `_iso/PlCa.dat`, `_iso/PlMs.dat`, `_iso/PlSk.dat`,
  `_iso/PlZd.dat` (source)
- `data/characters/fox.json`, `data/characters/falco.json`, `data/characters/falcon.json`,
  `data/characters/marth.json`, `data/characters/sheik.json`, `data/characters/zelda.json`
...
- `data/motion_state/owners/<char>.bin` (per-supported-character MotionState owner/callback tables;
  ...
  - Sources:
    ...
    - `refs/melee/src/melee/ft/chara/ftMars/ftMs_Init.c::ftMs_Init_MotionStateTable`
  ...
  - Class-bit examples:
    ...
    - `FT_CHECK_GROUND_LEDGE_AIR_COLL` is keyed to collision callbacks whose decomp bodies call
      `ft_CheckGroundAndLedge(gobj, CLIFFCATCH_*)` directly, including Fox/Falco aerial Side-B and
      Up-B states, Falcon Dive's airborne branch, Sheik Vanish, and common MissFoot/Pass. Runtime
      uses this callback-owner bit for the shared direct `mpColl_800473CC` / `mpColl_800471F8`
      floor and airborne wall-envelope paths without keeping a local action-family list.
      `FT_CHECK_GROUND_LEDGE_BOTH_COLL` narrows the callbacks that pass `CLIFFCATCH_BOTH`.
    ...
    - `class3_bits` is the narrow Phase 4 later-owner word plus walljump/wall-tech and source
      MotionState-entry ledgers. `PHASE4_ATTACK_AIR_COLL`, `PHASE4_ESCAPE_AIR_COLL`,
  ...
  - Stale/non-v23 `MSLMSO01` tables must be rejected. Version 23 adds conditional Falcon
    ...
    extends the generated callback classes for Falcon's source collision owners and common
    `FallAerial`. Version 20 adds `CLIFF_HOLD_PHYS_SNAP` class2 ownership. Version 19 adds the
    generated Walk/Fall/Guard/Catch/ground-locomotion/ledge/landing/grounded-attack destination
    class2 owners. Version 18 adds the `fx_special_kind` u8 lane (anim-callback-derived Fox/Falco
    special row identity, the is_spacie/action-id predicate migration target) and retires v17's
    transitional `FX_SPECIALHI_HOLD_AIR_PHYS` class3 bit in its favor. Version 17 added that bit as
    the migration exemplar. Version 16 adds `class3_bits` for narrow
...
- `data/stages/bin/{grnla,grnba,griz,grps,grst,grop}.bin` (stage collision/metadata; decomp-first, compact binary)
  ...
  - FoD seed lanes:
    ...
    - `floor_skip_segment_id_u16[4]`, `floor_skip_segment_valid_u8[4]` for hidden
      ...
      `bindings/msl_validation_specials.c::msl_derive_sheik_vanish_floor_skip_segments_py`,
      called by `tools/slippi/validation_buffer_builder.py::_derive_sheik_vanish_floor_skip_segments`.
...
- `data/model_parts/<char>.bin` (per-supported-character fighter part/anchor descriptors; compact
  binary)
  - Purpose:
  ...
  - Binary layout: `MSLPART1` v1
    - `u8 magic[8] = "MSLPART1"`
    - `u32 version = 1`
    - `u16 char_id`, `local_part_count`, `anchor_count`, reserved. `char_id` is the Slippi/CSS
      external-character domain used by extracted fighter archives (Falcon=0, Fox=2, Marth=9,
      Zelda=18, Sheik=19, Falco=20), not the simulator seed's internal FighterKind domain.
...
- `data/stage_items/dream_whispy.bin` (Dream Land Whispy wind data; generated `MSLWHSP1` compact binary)
  ...
  - Stale/non-v1 tables must be rejected by tooling readers; regenerate with
    ...
    or through `tools.extraction.build_data`.
- `data/scripts/<char>.bin` (per-supported-character unified decoded fighter script timeline;
  compact binary)
  - Purpose:
  ...
  - Binary layout: `MSLFTSC1` v3 (byte layout unchanged from v2; v3 requires typed
    `set_hitbox_interaction` events)
    - `u8 magic[8] = "MSLFTSC1"`
    - `u32 version = 3`
  ...
  - Generated/ignored; stale/non-v3 runtime tables must be regenerated. Python tooling readers may
    still accept legacy v1 JSON-payload tables for comparison/debug, but runtime C rejects them.
- `data/shields/<char>.bin` (guard-tilt shield bubble centers; decomp-first, compact binary)
- `data/hurtcaps/<char>.bin` and `.json` (hurt capsule init tables; JSON is the debug mirror; C
  loads `.bin` only)
- `data/hitboxes/<char>.bin` (hitbox event tables; decomp-first, compact binary)
- `data/moves/<char>.json` (subaction timelines for key motions and specials; decomp-first)
- `data/anims/<char>.bin` (`SSANIM01` v5 per-msid bone matrices plus TransN; decomp-first;
  CaptureCaptain cross-bakes Falcon's `PlCa.dat`/`PlCaAJ.dat` animation source onto each target
  skeleton)
- `data/anims/<char>.tracks.bin` (per-msid HSD AObj/FObj tracks; C collision-pose input)
- `data/anims/<char>.locals.bin` (per-msid JObj local SRT; C collision-pose input)
- `data/anims/<char>.dyn.bin` (`ftData.x2C` dynamic-chain descriptors; C collision-pose input)
- `data/anims/<char>.blend.bin` (blend/dynamics bytes; decomp-first)
- `data/anims_ecb/<char>.bin` (per-msid bone matrices plus TransN; **ECB extraction input only**;
  not read by the C core)
- `data/items/lasers.bin` (Fox/Falco blaster laser params; decomp-first, compact binary)
- `data/ecb/<char>_bottom.bin` (per-msid per-frame ECB bottom Y; decomp-shaped)
  - Format: `MSLECB01` v4. This version is tied to SSANIM01 v5 input semantics; stale v3
    derived tables are rejected by the runtime loader.
  - Source: `data/anims_ecb/<char>.bin` (SSANIM01 v5 matrices) + `data/characters/<char>.json` `ecb_joints`.
...
- `data/ecb/<char>_extents.bin` (per-msid per-frame ECB extrema; decomp-shaped)
  - Format: `MSLECB01` v5. This version is tied to SSANIM01 v5 input semantics; stale v4
    derived tables are rejected by the runtime loader.
  - Source: `data/anims_ecb/<char>.bin` (SSANIM01 v5 matrices) + `data/characters/<char>.json` `ecb_joints`.
...
- Source: each target's main/AJ/costume-rest DATs, `PlCo.dat` part tables, and Falcon donor
  `_iso/PlCa.dat` plus `_iso/PlCaAJ.dat` for `ftCo_SM_CaptureCaptain`.
...
Source and generation:
- Descriptor source: each target's main DAT and `PlCo.dat` part tables. Because the shared
  extractor emits SSANIM siblings in the same invocation, generation also requires the target AJ
  and costume-rest DAT plus Falcon donor `PlCa.dat`/`PlCaAJ.dat`.
- Generator: `tools/extraction/extract_fighter_anims.py`.
- Typical command:
  - `uv run python tools/extraction/extract_fighter_anims.py --character fox --moves data/moves/fox.json --out-dir data/anims`
  - `uv run python tools/extraction/extract_fighter_anims.py --character falco --moves data/moves/falco.json --out-dir data/anims`
...
## `data/hitboxes/<char>.bin` (MSLHITB1 v2)

The v2 byte layout is unchanged from v1. The version bump makes the low interaction-valid bits and
kind-3 `set_hitbox_interaction` mutations required generated semantics, so v1 tables fail freshness
instead of silently treating Falcon's source interaction changes as absent.
...
- Records (`rec_count` entries), each 44 bytes:
  ...
  - `kind: u8`:
    ...
    - `3`: active-slot `x42_b5` / `x42_b7` interaction mutation (`ftAction_80071708`) with no
      create-edge side effects
  ...
  - `u16_tail[8]: 8 * u16` additional extracted fields (decomp-shaped):
    ...
    - `u16_tail[6]`: bitfield of boolean flags:
      - bit 2: low interaction bits are valid (legacy synthetic records without this bit default to enabled)
      - bit 1: runtime `HitCapsule.x42_b7` item interaction gate
      - bit 0: runtime `HitCapsule.x42_b5` fighter interaction gate
      - bit 15: `rebound`
      - bit 14: `clank`
...
- Kind-3 events replace only the active slot's `x42_b5` / `x42_b7` interaction state. Fighter
  contact requires `x42_b5`; item-hurtbox contact requires `x42_b7`, matching `ftcoll.c` and
  `itcoll.c` respectively.
...
- Validation buffers store `ground_id` directly from Slippi post-frame `ground` (see `tools/slippi/validation_buffer_builder.py`).
...
`build_data` writes a runtime data root and therefore requires the complete character registry.
Use individual extraction modules for per-character debug artifacts; a partial character manifest
is not a native-runtime contract.
...
uv run python -m tools.extraction.iso_extract --iso SSBM.iso --glob '*ItCo.dat'
uv run python -m tools.extraction.iso_extract --iso SSBM.iso --glob '*Pl*.dat'
```

The fighter glob must include each registry character's main, AJ, and costume-0 DATs. In
particular, every SSANIM v5 build requires `PlCa.dat` and `PlCaAJ.dat` for CaptureCaptain's Falcon
animation donor, even when invoking the low-level animation extractor for another target.

`data/manifest.json::schemas` and native `data_schema_versions()` expose the same exact schema set:
`attack_id_move_id`, `ecb_bottom`, `ecb_extents`, `fighter_anims`, `fighter_hitboxes`,
`fighter_scripts`, and `motion_state_owners`. Missing or extra keys are contract mismatches.
