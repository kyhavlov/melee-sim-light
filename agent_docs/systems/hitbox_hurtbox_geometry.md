...
| `ftAction_80071A58`; `ftAction_80071A9C` hurt-state script effects | `extract_fighter_script_timeline.py`; `fighter_script.c`; `src/hurtboxes.c` | All-hurt and per-bone hurt state commands | `MSLFTSC1` events plus persistent live per-hurtcap state | CLOSED | Per-bone script effects use extracted hurtcap bone ids and the live BODY consumer. Locks: `test_hurtbox_state_from_hit_status.py`, `test_hurtbox_state_sim_owned.py`. |
