...
| Area | Status | Closure |
|---|---|---|
| HitCapsule create/clear/restart lifecycle | CLOSED | `MSLFTSC1`/`MSLHITB1` preserve source create, clear, active-slot damage mutation, authored hit group, and enable-edge victim-list copy/clear semantics. |
...
| Hitbox size and interaction mutation opcodes | RETAINED SOURCE-POLICY | Decomp owners are known, but current supported Fox/Falco extracted scripts do not emit `set_hitbox_size` or `set_hitbox_interaction`. If supported data begins emitting them, promote them to distinct `MSLHITB1` active-slot mutation records rather than folding them into create edges. |
...
| `ftAction_80071708` set HitCapsule `x42_b5` / `x42_b7` interaction flags | `extract_fighter_script_timeline.py`; `extract_fighter_hitboxes.py` | Active HitCapsule id, flag selector, value | Source owner identified; no current supported Fox/Falco script emits it | RETAINED SOURCE-POLICY | Future support should be a distinct active-slot interaction mutation, not a create record. Guard: `test_supported_scripts_do_not_emit_deferred_hitbox_geometry_mutations`. |
...
- `set_hitbox_size` and `set_hitbox_interaction` are source-known but absent from current supported
  Fox/Falco extracted scripts. They remain deferred only while data guard tests prove that absence.
