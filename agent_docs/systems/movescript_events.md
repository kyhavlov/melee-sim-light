...
| Deferred non-runtime fighter opcodes | RETAINED SOURCE-POLICY | `ftAction_8007169C` set hitbox scale and `ftAction_80071708` set hitbox interaction remain deferred unless supported scripts emit them. `remove_hitbox` is folded in `MSLHITB1` hitbox cache where it affects active hitbox publication. |
