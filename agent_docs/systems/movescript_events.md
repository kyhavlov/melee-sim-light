...
| Hitbox create/clear/damage lifecycle | CLOSED | `MSLFTSC1` carries create/clear and opcode-12 damage mutations; `MSLHITB1` preserves damage mutations as distinct active-slot mutation records so runtime hitbox refresh updates damage without create-edge side effects. |
...
| Deferred non-runtime fighter opcodes | RETAINED SOURCE-POLICY | `set_hitbox_size`, `set_hitbox_interaction`, and `remove_hitbox` remain deferred unless a supported runtime owner consumes them. `remove_hitbox` is folded in `MSLHITB1` hitbox cache where it affects active hitbox publication. |
