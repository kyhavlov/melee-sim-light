...
| Rollout RNG frame clock | `msl_batch_commit_rollout_clock_rng` after `step_one_frame`; explicit source stream lanes | HSD RNG seed, frame-owner policy, hidden stream-phase reconstruction | Seed-owned frame clock by default; source-owned lanes for Shy Guys, DamageFlyRoll/Fighter_8006CDA4, match-flow RNG where modeled | RETAINED SOURCE-POLICY | Default seed-owned replay clock is intentional. Locks include DamageFlyRoll RNG phase tests, Yoshi Shy Guy RNG tests, and match-flow RNG tests. |
...
|---|---|---|
| Replay seed-owned frame RNG by default | RETAINED SOURCE-POLICY | Replay rows expose `frame_pre_random_seed`; generic rollout should not invent scheduler RNG ownership unless a source-modeled phase consumes RNG. |
