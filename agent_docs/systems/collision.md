...
| `MSL_MPCOLL_REJECT_SUSTAINED_ESCAPEAIR_SAME_LEDGE_LOCK` | RETAINED SOURCE-POLICY | Sustained EscapeAir ledge-floor rows remain airborne until the source ledge/floor handoff has enough depth or allow-interrupt ownership. | Legal-stage ledge EscapeAir locks. |
...
| `MSL_MPCOLL_REJECT_KNEEBEND_ESCAPEAIR_SLOPE` | RETAINED SOURCE-POLICY | Fresh KneeBend -> EscapeAir ledge floors require a source floor producer before final publication. | EscapeAir ledge owner tests. |
| `MSL_MPCOLL_REJECT_JUMPAERIAL_ESCAPEAIR_HIGH_LIFT_LEDGE` | RETAINED SOURCE-POLICY | JumpAerial -> EscapeAir high-lift ledge remaps are a separate entry-lifetime suppression; carried cliff ledge floors use the shared authority packet rather than this guard. | JumpAerial/EscapeAir ledge locks. |
...
| `MSL_MPCOLL_REJECT_CLIFF_HORIZONTAL_LEDGE_LOCKED` | RETAINED SOURCE-POLICY | Cliff/EscapeAir carried ledge floors stay airborne until source ledge/floor handoff permits final LandingFallSpecial. | Cliff/ledge EscapeAir locks. |
