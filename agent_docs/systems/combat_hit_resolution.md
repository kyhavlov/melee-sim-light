...
| Item/projectile shared boundary | CLOSED | Shared item/projectile contacts enter the same ProcessHit/shield/stale substrate. Fox/Falco laser zero-KB damage class is source-derived from the article HitCapsule KB tuple and closes the shared combat boundary for supported laser BODY hits. |
...
| Item/projectile shared BODY and SHIELD boundary: `itcoll.c::it_80272460`, `ftColl_80077688`, `ftColl_80077C60`, item stale helpers | `src/combat.c::combat_apply_item_hit`, `combat_apply_item_body_hit`, `combat_apply_item_shield_hit`, item post-combat callbacks | Item owner, reflected damage, item hitbox damage, item attack id/instance, fighter victim state, zero applied-KB item damage class | Shared combat damage products and shield application helpers; MSLLASR1 `zero_kb_damage_class` is source-derived from supported laser article HitCapsule KB terms | CLOSED | Shared shield/BODY/stale substrate is modeled. Laser zero-KB damage-class gating is proven from source: item scripts write kbg/wsk/bkb into HitCapsule.x24/x28/x2C, item BODY contact writes percent-temp in `ftColl_80077C60`, and `Fighter_ProcessHit_8006D1EC` enters Damage* only when applied KB is nonzero. Fox laser states carry all-zero KB terms; Falco states carry nonzero KB terms and use normal flinching BODY. Detailed item/projectile lifecycle remains item/projectile scope. |
...
| Item/projectile shared-boundary only | RETAINED SOURCE-POLICY | Item/projectile contacts can enter shared `ProcessHit`/shield/stale logic, and supported Fox/Falco laser zero-KB BODY hits are closed through MSLLASR1 source-derived KB terms. Item lifecycle, projectile ownership, reflection/bounce, and item-only clank remain separate system work. |
