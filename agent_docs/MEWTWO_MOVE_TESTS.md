...
- All 12 diagnostic Mewtwo recordings now pass exactly: 123,473 transitions,
  no mismatches or runner errors. Two July captures require the explicitly
  selected legacy Dolphin arithmetic profile; the other ten retain metadata
  defaults. See [MEWTWO_FNMSUBS.md](MEWTWO_FNMSUBS.md) for traced operands,
  regression fixtures, and the distinction from an independently known build.
...
captures now cover Teleport-to-ledge recovery, Falco laser/Samus missile reflection,
and partial/full Shadow Ball absorption; see
