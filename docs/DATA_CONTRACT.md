...
- For DamageFlyTop segments, nonzero consume counts may be carried backward across the same
  replay-visible DamageFlyTop/source-owner segment. This models persistent hidden
  `Fighter_8006CDA4` held-item / x197C branch state so rollouts seeded before the later hit have the
  stream phase without runtime action-frame inference.
