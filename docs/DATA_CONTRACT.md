...
- The collision-owner index is the only runtime predicate for dynamic matrix substitution. Current
  generated data marks Fox `ftCo_SM_AttackHi3` (`submotion_id=58`) because pre-`ftColl_80078C70`
  Dolphin primitive probes show that submotion's BODY hurtcap endpoints consume the live dynamic
  chain, while broad application to other common attacks regressed protected BODY rows. C gameplay
  must not hardcode this msid; adding another dynamic collision submotion is an extraction/data
  contract change backed by the same primitive evidence.
