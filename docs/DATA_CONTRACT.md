...
- The terminal counter value is one-step teacher-forced from `ref_t1.percent` because
  `Camera_80031144` and `Player_GetMoreFlagsBit3` are hidden replay state. Non-terminal rows remain
  below the damage threshold unless the next replay row shows the source-shaped magnify tick with no
  action change, no hitlag/hitstun transition, and no visible hit-source attribution change.
...
- Replay-seeded validation rollouts do not consume this approximation. The RL1 profile intentionally
  ignores the camera-box visibility bit, so using that bit as live rollout damage evidence would turn
  an ignored camera lane into a gameplay break. Teacher-forced one-step seeds still use the lane.
...
- For replay-proven AttackAirN pre-action delayed gates, only nonzero consume counts `1..3` may be
  ...
  hidden branch state for that owner.
